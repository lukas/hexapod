#!/usr/bin/env python3
"""Guarded runner for experiment d4908236 (L2 belly-rest radial shear
hysteresis, 6 repeats).

REUSED UNCHANGED except for this header from the runner that executed the
sibling experiment 413d5402 at 02:58Z on the same day, against the same
reviewed protocol hash a99ceef28136 and the same installed robot revision.
Keeping it byte-identical is the point: the two runs are then directly
comparable and the earlier one is this one's own executor validation.

Executes the reviewed sysid protocol through the robot's own deterministic
executor (POST /api/sysid/run, force=true) while holding the EXCLUSIVE COMMAND
LEASE this experiment lists as a dependency, and while an independent
supervisor enforces the bounds the saved plan asks for.

Design carried over from the L5 sibling run (b289e536), whose attempt 1 was
limped by its own supervisor: `/api/feedback` is a full 18-servo bus scan
costing ~5 s and starves the runner's position read, so bus telemetry is read
PRE/POST only.  The real-time layer is the runner's own in-loop interlocks
(0.75 A over 3 polls, 3.0 A ceiling, 55 C, 30 deg tracking vs a slewed
reference, 3 missed reads, stale state, limp-on-trip); supervision is the
bus-free continuous camera record plus a freshness watchdog plus the lease.

New here vs the L5 script:
  * holds /api/command-lease for the whole window and passes the token on the
    run POST, so no other controller can command motion into the middle of it;
  * treats a lease refusal (a foreign motion command, refused before the bus)
    as the plan's `foreign_controller_command_observed` stop;
  * verifies the start pose immediately before the POST -- the runner glides
    to home_deg from wherever it is, so an unnoticed stand would turn this
    into an unplanned lower;
  * appends frames/markers/telemetry to JSONL as they happen instead of only
    dumping at exit, so a killed supervisor still leaves the evidence.
"""
...
LEASE_OWNER = os.environ.get("LEASE_OWNER", "guarded-runner-d4908236")
...
MOVING_LEG = "L2"
HIP_J, KNEE_J, YAW_J = 7, 8, 6          # L2 hip/knee/yaw logical joints
KNEE_RISE_TRIP_C = 8.0                  # plan: L2 knee > +8 C over baseline
TEMP_TRIP_C = 55.0
TILT_TRIP_DEG = 10.0
CURRENT_TRIP_A = 3.0
CAMERA_STALL_S = 2.0
...
def healthy_samples(n=3, tries_max=12, tag="prearm"):
    ...
    while len(got) < n and tries < tries_max:
        ...
        s = {"t": time.time(), "live": 18,
             ...
             "l2_knee_temp_c": js[KNEE_J]["temp_c"],
             "l2_hip_temp_c": js[HIP_J]["temp_c"],
...
def main() -> int:
    ...
    try:
        ...
        if len(pre) < 3:
            ...
            return 2
        knee_base = min(s["l2_knee_temp_c"] for s in pre)
        print(f"   pre-arm 18/18 x3, maxtemp="
              f"{max(s['max_temp_c'] for s in pre)}C, L2 knee baseline "
        ...
        knee_peak = max((s["l2_knee_temp_c"] for s in post_s), default=None)
        ...
        summary = {
            ...
            "l2_knee_baseline_c": knee_base, "l2_knee_post_peak_c": knee_peak,
            "l2_knee_rise_c": knee_rise,
            "l2_knee_rise_trip_c": KNEE_RISE_TRIP_C,
