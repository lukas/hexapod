#!/usr/bin/env python3
"""Documented re-pose recovery before experiment d4908236.

The robot was left off `belly_rest_logical_zero` by hands at ~03:22Z (see
artifacts/l2_belly_rest_hysteresis_20260910/POST_RUN_STATE_CHANGE.md).  The
...
def main():
    ...
    r = post("/api/command-lease/acquire",
             {"owner": OWNER, "ttl_s": 600.0,
              "reason": "restore belly_rest_logical_zero before d4908236"},
             lease=False)
    if not r.get("ok"):
