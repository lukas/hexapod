#!/usr/bin/env python3
"""Guarded runner for experiment 7b565e2a (L3 belly-rest radial shear
hysteresis, 6 repeats).

DERIVED FROM the runner that executed 881677a0 (L0) at 14:52Z on 2026-09-10
and completed 1560/1560 ticks, which came from the 9e65cca8 (L1) runner and
before that d4908236 (L2). Same reviewed protocol FAMILY, and the SAME
installed robot revision as the L0 run -- 71459673, deployed 14:48:25Z:
sysid_runner.py, sysid_protocol.py, command_lease.py, mcu_feetech_bus.py,
web_server.py and async_bus_guard.py are byte-identical between this
checkout and what is installed, so nothing was deployed for this run.

ONE kind of change against the L0 runner, so the diff is auditable:

  A. Retarget to L3 -- MOVING_LEG, the hip/knee/yaw joint indices
     (10/11/9 instead of L0's 1/2/0), the baseline-temperature key names,
     the lease owner. Nothing else.

Both of the L0 runner's other deliberate changes are carried over UNCHANGED
because this plan asks for exactly the same thing L0's did:

  B. START_POSE_TOL_DEG stays 1.0. This saved plan also sets
     `preflight.verify_start_pose_within_deg_of_logical_zero: 1.0`.

  C. The narrow /api/errors tolerance stays as L0 shipped it. The plan's
     ...
     On the L0 run this allowance was spent exactly once, on the row
     class it was written for, and it was the difference between a
...
LEASE_OWNER = os.environ.get("LEASE_OWNER", "guarded-runner-7b565e2a")
...
MOVING_LEG = "L3"
HIP_J, KNEE_J, YAW_J = 10, 11, 9        # L3 hip/knee/yaw logical joints
KNEE_RISE_TRIP_C = 8.0                  # plan: L3 knee > +8 C over baseline
TEMP_TRIP_C = 55.0
TILT_TRIP_DEG = 10.0
CURRENT_TRIP_A = 3.0
CAMERA_STALL_S = 2.0
...
def healthy_samples(n=3, tries_max=12, tag="prearm"):
    ...
    while len(got) < n and tries < tries_max:
        ...
        s = {"t": time.time(), "live": 18,
             ...
             "l3_knee_temp_c": js[KNEE_J]["temp_c"],
             "l3_hip_temp_c": js[HIP_J]["temp_c"],
...
def main() -> int:
    ...
    try:
        ...
        if len(pre) < 3:
            ...
            return 2
        knee_base = min(s["l3_knee_temp_c"] for s in pre)
        print(f"   pre-arm 18/18 x3, maxtemp="
              f"{max(s['max_temp_c'] for s in pre)}C, L3 knee baseline "
        ...
        knee_peak = max((s["l3_knee_temp_c"] for s in post_s), default=None)
        ...
        summary = {
            ...
            "l3_knee_baseline_c": knee_base, "l3_knee_post_peak_c": knee_peak,
            "l3_knee_rise_c": knee_rise,
            "l3_knee_rise_trip_c": KNEE_RISE_TRIP_C,
