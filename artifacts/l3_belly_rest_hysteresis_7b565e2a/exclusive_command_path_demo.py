#!/usr/bin/env python3
"""Re-demonstrate the exclusive command lease for THIS run (7b565e2a).

The plan's analysis_dependencies[1] asks for the lease to be "demonstrated to
refuse :8898 hub and direct motion routes while leaving abort paths open,
reusing the mechanism recorded in exclusive_command_path_demo.json".  That
recorded demo belongs to 413d5402; this reproduces it under this run's own
owner so the evidence is this experiment's, not a citation.
...
def call(base, path, body=None, method="POST", hdrs=None):
    req = urllib.request.Request(
        ...
        headers={"Content-Type": "application/json",
                 "X-Hexapod-Controller": "exclusivity-probe-7b565e2a",
...
code, body, ms = call(ROBOT, "/api/command-lease/acquire",
                      {"owner": OWNER, "ttl_s": 120,
                       "reason": "exclusivity demonstration for 7b565e2a"})
