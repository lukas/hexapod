#!/usr/bin/env python3
"""Guarded runner for experiment 7299f243 (L4 belly-rest radial shear
hysteresis, 6 repeats) -- the sixth and final leg of the belly-rest family.

DERIVED FROM the runner that executed 022f2098 (L3 replicate) at 16:04Z on
2026-09-10 and completed 1560/1560 ticks with zero /api/errors rows, which
came from the 881677a0 (L0) runner, and before that 9e65cca8 (L1) and
d4908236 (L2). Same reviewed protocol FAMILY, and the SAME installed robot
revision the L3 run used -- sysid_runner.py, sysid_protocol.py,
command_lease.py, mcu_feetech_bus.py, web_server.py and async_bus_guard.py
are byte-identical between this checkout and what is installed, so nothing
is deployed for this run. That identity is also the current evidence that
resolves the saved plan's creation-time `_adaptive_admission.ready: false`,
whose two stated reasons were unproved runtime compatibility and no named
trusted deterministic executor: the executor is POST /api/sysid/run,
sysid_protocol v1, which has now run L5, L2, L1, L0 and L3 byte-exact on
this same revision.

ONE kind of change against the L3 runner, so the diff is auditable:

  A. Retarget to L4 -- MOVING_LEG, the hip/knee/yaw joint indices
     (13/14/12 instead of L3's 10/11/9), the baseline-temperature key
     names, the lease owner. Nothing else.

     The saved plan names hip_joint 13, knee_joint 14, yaw_joint 12, and
     the generated protocol's varying columns are exactly {13, 14} with
     j12 constant, so the runner's guarded scope and the protocol's own
     moving joints agree. This is checked rather than assumed: the L3 run
     had to publish a correction because its declaration described the
     in-loop active-joint scope with L0's inherited indices.

Both of the L3 runner's other deliberate changes are carried over UNCHANGED
because this plan asks for exactly the same thing L3's did:

  B. START_POSE_TOL_DEG stays 1.0. This saved plan also sets
     `preflight.verify_start_pose_within_deg_of_logical_zero: 1.0`.

  C. The narrow /api/errors tolerance stays as shipped. The plan's
     /api/errors stop is written as
     "any new /api/errors row BEYOND a single self-recovered transient
     bus_timing retry". This runner implements the plan as written, and
     narrowly:
     ...
     The L0 run spent this allowance exactly once; L3's replicate did not
     spend it at all.
...
preflight_declaration.json, per analysis_dependencies[6].
"""
...
LEASE_OWNER = os.environ.get("LEASE_OWNER", "guarded-runner-7299f243")
...
MOVING_LEG = "L4"
HIP_J, KNEE_J, YAW_J = 13, 14, 12       # L4 hip/knee/yaw logical joints
KNEE_RISE_TRIP_C = 8.0                  # plan: L4 knee > +8 C over baseline
TEMP_TRIP_C = 55.0
TILT_TRIP_DEG = 10.0
CURRENT_TRIP_A = 3.0
CAMERA_STALL_S = 2.0
...
def healthy_samples(n=3, tries_max=12, tag="prearm"):
    ...
    while len(got) < n and tries < tries_max:
        ...
        s = {"t": time.time(), "live": 18,
             ...
             "l4_knee_temp_c": js[KNEE_J]["temp_c"],
             "l4_hip_temp_c": js[HIP_J]["temp_c"],
...
def main() -> int:
    ...
    try:
        ...
        if len(pre) < 3:
            ...
            return 2
        knee_base = min(s["l4_knee_temp_c"] for s in pre)
        print(f"   pre-arm 18/18 x3, maxtemp="
              f"{max(s['max_temp_c'] for s in pre)}C, L4 knee baseline "
        ...
        knee_peak = max((s["l4_knee_temp_c"] for s in post_s), default=None)
        ...
        summary = {
            ...
            "l4_knee_baseline_c": knee_base, "l4_knee_post_peak_c": knee_peak,
            "l4_knee_rise_c": knee_rise,
            "l4_knee_rise_trip_c": KNEE_RISE_TRIP_C,
