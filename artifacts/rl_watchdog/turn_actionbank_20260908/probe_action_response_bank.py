...
def _rollout(spec: dict, *, vx: float, wz: float, phase_offset: float,
    ...
    try:
        for step in range(n_ticks):
            ...
            # step info has no pitch_rel_deg key; absolute pitch_deg is
            # honest here because every retention screen is baseline-
            # relative (the tilt reference cancels in the comparison).
            pitches.append(float(info.get("pitch_deg") or 0.0))
            if record_qpos:
