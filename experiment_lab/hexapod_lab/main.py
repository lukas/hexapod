...
def create_app(settings: Optional[Settings] = None) -> FastAPI:
    ...
    def dashboard(request: Request, principal: Principal = Depends(viewer)):
        ...
        return page("Hexapod Lab", f"<div class='dashboard-head'><div><h1>Hexapod Lab</h1><p class='lede'>Experiment queue and durable run evidence</p></div>{tools}</div><main>{queue_panel}{runner_panel}{robot_status_panel()}<h2>Experiments</h2>{cards}</main>")
