...
def create_app(settings: Optional[Settings] = None) -> FastAPI:
    ...
    def dashboard(request: Request, principal: Principal = Depends(viewer)):
        ...
        return page("Hexapod Lab", f"<div class='dashboard-head'><div class='dashboard-title'><h1>Hexapod Lab</h1><p class='lede'>Experiment queue and durable run evidence</p></div>{tools}</div><main>{queue_panel}{runner_panel}{robot_status_panel()}<h2>Experiments</h2>{cards}</main>")
...
def codex_queue_panel(control, can_resume):
    if not control.get("paused"):
        # Healthy is the normal case and it does not deserve a panel. A
        # heading, a paragraph of explanation and an always-open pause form
        # pushed the experiment list below the fold, which is the one thing
        # an operator opens this page to see.
        return (
            "<section class='queue-strip'>"
            f"<span class='queue-strip-state'>{escape(agent_label())} "
            "experiment loop running</span>"
            "<span class='queue-strip-note'>queue latch clear</span>"
            + (
                "<details class='queue-strip-pause'><summary>Pause…</summary>"
                + pause_controls()
                + "</details>"
                if can_resume else ""
            )
