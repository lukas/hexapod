...
def test_agent_runner_identity_resolves_a_cli_from_path(tmp_path, monkeypatch):
    runner = tmp_path / "claude"
    runner.write_bytes(b"configured provider binary")
    runner.chmod(0o700)
    monkeypatch.setenv("PATH", str(tmp_path))
    monkeypatch.setattr(
        codex_module.subprocess, "run",
        lambda *_args, **_kwargs: subprocess.CompletedProcess(
            [], 0, "test-provider-version", ""
        ),
    )
    identity = _codex_runner_identity(Path("claude"))
    assert identity["runner_resolved_path"] == str(runner.resolve())
    assert identity["runner_sha256"] == hashlib.sha256(runner.read_bytes()).hexdigest()
