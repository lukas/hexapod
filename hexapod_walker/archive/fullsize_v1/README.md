...
```bash
# from the repo root
./run.sh hexapod_walker/fullsize_v1/hexapod_walker.py        # 11 fabricable STLs
./run.sh hexapod_walker/fullsize_v1/build_full_assembly.py   # full-assembly category STLs
./hexapod_walker/fullsize_v1/render_blender.sh \              # photoreal Cycles render
    --device METAL --samples 256 \
    --out hexapod_walker/fullsize_v1/renders/walker_hero.png
```

## Generate the fabricable STLs

```bash
./run.sh hexapod_walker/fullsize_v1/hexapod_walker.py
```

This creates `hexapod_walker/fullsize_v1/stl/` containing:
...
```bash
./run.sh hexapod_walker/fullsize_v1/build_full_assembly.py
```
...
```bash
./hexapod_walker/fullsize_v1/render_blender.sh                                # default
./hexapod_walker/fullsize_v1/render_blender.sh --device METAL --samples 256   # GPU + nicer
./hexapod_walker/fullsize_v1/render_blender.sh --camera-azimuth-deg 90        # side view
```
...
After editing, re-run `./run.sh hexapod_walker/fullsize_v1/hexapod_walker.py` —
...
./run.sh hexapod_walker/fullsize_v1/mujoco_walker.py --vx 0.4 --omega 0.05

# headless: simulate N seconds, print chassis trajectory
./run.sh hexapod_walker/fullsize_v1/mujoco_walker.py --headless --duration 8.0 --vx 0.4

# strafe sideways, curve, spin in place, etc.
./run.sh hexapod_walker/fullsize_v1/mujoco_walker.py --vy 0.4                    # +Y strafe
./run.sh hexapod_walker/fullsize_v1/mujoco_walker.py --vx 0.3 --omega 0.10       # left curve
./run.sh hexapod_walker/fullsize_v1/mujoco_walker.py --omega 0.15                # spin

# disable terrain / pick a different world / change obstacle layout
./run.sh hexapod_walker/fullsize_v1/mujoco_walker.py --no-terrain
./run.sh hexapod_walker/fullsize_v1/mujoco_walker.py --terrain-seed 7
./run.sh hexapod_walker/fullsize_v1/mujoco_walker.py --obstacles 30           # busier
./run.sh hexapod_walker/fullsize_v1/mujoco_walker.py --obstacles 0            # clear
./run.sh hexapod_walker/fullsize_v1/mujoco_walker.py --obstacle-seed 42
...
./.venv/bin/python hexapod_walker/fullsize_v1/eval_walker.py \
    --policy hexapod_walker/fullsize_v1/policies/walker_v9/walker_v9.zip --quiet

# robustness sweep under mass / friction / latency / bias / vel-kick
./.venv/bin/python hexapod_walker/fullsize_v1/eval_perturbed.py \
    --policies baseline \
        hexapod_walker/fullsize_v1/policies/walker_v6/walker_v6.zip \
        hexapod_walker/fullsize_v1/policies/walker_v8/walker_v8.zip \
        hexapod_walker/fullsize_v1/policies/walker_v9/walker_v9.zip
```
...
# walker_v11 — narrow-stance variant of v10.  ``--stance-radius-scale
#              0.70`` shrinks the foot's body-frame radial distance to
#              70 % of the wide spider stance, producing a more
#              compact dog-like silhouette.  The IK absorbs the
#              change as deeper knee bend; the chassis stays at the
#              same height because the foot's z-below-hip is preserved.
#              Knee + pitch joint ranges in the MJCF were widened
#              accordingly (knee 1.7 → 2.7 rad, pitch lower bound
#              -1.4 → -1.6 rad) -- existing policies stay inside the
#              old ranges so they're unaffected.
./.venv/bin/python hexapod_walker/fullsize_v1/train_walker.py \
    --steps 6000000 --n-envs 8 --tag walker_v11 \
    --gait-action --per-leg-lift --stub-w 0.5 \
    --stance-radius-scale 0.70 \
    --episode-seconds 16.0 \
    --cmd-resample-seconds 5.0 \
    --terminate-on-stuck-seconds 4.0 \
    --obstacle-count 20 \
...
| `walker_v11`  | narrow-stance variant of v10; legs tucked tight under the chassis (stance radius 70 % of the wide spider stance) for a more compact, dog-like silhouette. |
