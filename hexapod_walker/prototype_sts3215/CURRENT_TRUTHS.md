...
- The scripted gait's stop leaves the swing tripod mid-stride (legs 0, 2, 4 read knees 48-102 and hips to 39 against 80/20), so the next J command is refused with "not at walk-ready pose (max dq=32 on j2)" until the robot is re-stood; hexapod 2 recorded the same on 2026-09-11 (run e3d29d962f0d). Not a horn or a servo fault: the encoders read what the gait commanded, and the 120 s hold sags under 0.8 deg. (An earlier note today called this an L0 knee horn slip; retracted.) The lab's walk runner now re-stands once and retries when a command is refused for walk-ready.
...
**Afternoon update (2026-09-11 ~15:00):** two more six-leg passes (robot moved, then turned ~190 deg with the gait) merged in. Faces 42 and 21 are on leg 5, not legs 0/1 as the old layout said; 62, 19, 27 confirmed. Leg 5 knee -y is the mount whose tag fell off (bare side plate in the zoom, `tag-calibration-20260911-full3/zoom_leg5_knee.jpg`). Four mounts have no known tag: L0 hip +y and -y, L1 knee +y, L5 knee -y; six faces (16, 20, 31, 33, 51, 61) still carried `verified: false`. Hazard: a bag of spare printed tags (copies of 3 and 4) inside camera 2's view; both the camera server and the tool now drop an id decoded twice in one frame and list it (`duplicate_ids`). Move spare tags out of the frame. Chassis euler z: 12.2 (six-leg pass wins over passes that saw fewer legs). Gait turn rate at omega 0.5 was ~3 deg/s.
...
**Full report:** `docs/GAIT_SIM_VS_HARDWARE_2026-09-11.md` (tables per gait with run ids, sim model family per number, what is still missing). **Metric script:** `sysid/gait_metrics.py FILE.csv [--phase walk|--seg 0]` — duration, tick rate, cmd/measured amplitude by joint type, tracking error, stride Hz from knee/hip commands, per-leg knee swing, tilt, current. **Per-run notes and contact sheets** are filed in Robot Lab v2 (dashboard `https://robot-lab.cwd1f0-new-cluster.coreweave.app`, MCP `get_run`/`add_finding`); hexapod2 runs `c728d29657ef` (allheading 100 Hz + dep_tip1), `e9ab4b1873c7` (walkteach, AMP), `8c28b0dbd205` (scripted GAIT 1, cap29/stotight45 attempts); hexapod1 runs `ec02c879c8f2`, `35d88e1cb7c3` (tripod step in place), `cb3b0231e0e9` (lift ladder), `5f79e4b7dd71` (weight shift).

**Same as sim (rulings you can lean on):**
- Stride frequency transfers exactly: 1.333 Hz configured -> 1.32–1.35 Hz measured on both 100 Hz policies; AMP 1.1 -> 1.08. The gait clock is not where the gap is.
- acq1_stdanneal: ~1 m in 31 s forward on camera (≈0.032 m/s) vs full-mesh (3.49 kg) sim vx median 0.031; same tucked low stance in the sim rollout frames and on the floor camera. Its slowness is the policy (progress ratio 0.42 at 0.08 cmd), not transport or hardware.
- Both robots' RL gaits shuffle (knee swing 8–13 deg, feet dragged in swing); sim never charged for it.

**Different from sim:**
- Tracking error: hardware 3.3/11.7 (allheading), 4.6/11.5 (walkteach), 8.7/32.1 (AMP), 7.2/15.8 (dep_tip1) deg mean/max; sim `sim_model_loaded.json` fit is sub-degree. Worst on the legacy 25 Hz policies with 48–60 deg swings.
- Tilt: RL gaits roll/pitch ±3 deg (100 Hz) to 7–10 deg (AMP, dep_tip1); scripted gait sim 0.2–0.5 deg vs 10.3 deg hardware IMU peak (hexapod1 gait 14, Sep 3).
- Open-loop travel ratio: sim OPTIMISTIC for no-slip gaits (gait 14: 0.84 sim vs 0.6–0.7 hexapod1; GAIT 1 on hexapod2 0.3–0.4 at 30 mm/s, camera px-scale estimate) and PESSIMISTIC for the drag gait (`GAIT.md`: 0.35–0.41 sim vs 0.50 tape). Same floor, opposite sign -> contact model, not friction.
- AMP `phasehz11_s29`: sim det progress 1.06 (legacy primitive model) arrived on hardware as 32 deg hip swings, 6.9 deg tilt, heading drift, and leg 3 swinging its knee 8.8 deg vs 18–31 on the other five. Not a speed win.
- hexapod1 loaded droop: weight-bearing hips settle 2–4 deg short of command (tripod step-in-place cmd 10.5 deg hip amp -> 8.8 measured, reproducible across two runs); at 15 mm commanded lift the camera saw NO clearance, the 45 mm ladder rung did. No sim counterpart exists: `sysid/replay.py` is fixed-base with contacts off. Any sim foot lift under ~20 mm is fiction on this robot.

**Caveats that change what you may conclude:** (1) three sim families — full mesh 3.49 kg (Mac only), checked-in twin 4.81 kg from 09-03 to 09-11 (see the mass-audit entry above; cap29's 50/100 Hz numbers are on it and must be re-run), legacy primitive (dep_tip1, stotight45, amp). Only acq1_stdanneal has a directly recorded sim m/s; every other sim speed in the report is progress_ratio x command. (2) walkteach and AMP hardware speeds were never tag-tracked (visual estimate only; the suite tracker ran only for scripted gaits). (3) `rl_docs` tracking says amp/cap29/stotight45 never ran on hardware — they did, on hexapod2 on 09-10 (cap29 and stotight45 were guard-aborted in 2–3 s, so "not measured", not "slow"); the lab DB is the record. (4) Nobody computed a smoothness metric before this; `gait_metrics.py` is the start — run it after every run and file the numbers with `hexapod-lab2 note`.

## MASS-AUDIT BUG FOUND + FIXED: the checked-in `mesh_mjx_twin` (what EVERY GPU pod actually trains/evals on) has been **37.6% heavier than the intended/real robot since 2026-09-03** — the whole 09-03..09-11 standwalk over_current saga was very likely fighting this bug, not a genuine physics/reward-pricing ceiling (2026-09-11 ~12:1x, cross-track, refill cycle)

One plain sentence: the file every GPU-trained checkpoint of the last ~8 days actually ran on secretly weighed 4.81 kg instead of the documented, CAD-audited, real 3.49 kg robot, and once you fix the file, the standwalk stall-fight this whole cycle-chain spent a full day trying to reward-engineer away (`k_current_hot`/`k_load_even`/`k_torque_headroom`, 12+ arms, all "closed short of PASS") disappears immediately with ZERO extra training.

**What the bug was:** `resolve_model_source`'s docstring (`servo_model.py`) and the `mesh`-family design (2026-08-24, `35033fac`) promise the checked-in `mesh_mujoco/hexapod_mesh_mjx.xml` (what every GPU pod loads — no pod has the gitignored full-CAD STL assets) has "SAME kinematics, masses and inertia" as the full-mesh CAD model. Verified compiling both today: `hexapod_mesh.xml` (full mesh) = **3.490 kg**, checked-in `hexapod_mesh_mjx.xml` (the twin) = **4.806 kg** (+37.6%). `git log` shows the twin was regenerated and committed standalone at `a55173ab` (2026-09-03, "Improve RL feel-test and checkpoint workflows") with NO corresponding change to `build_mesh_model.py` — someone ran the generator against a locally-modified, never-committed version of the mass tables, committed only the output artifact, and the checked-in generator + the twin have disagreed ever since. Every individual body's inertial got heavier (chassis 1.473kg->2.062kg, each yaw servo 0.161kg->0.233kg, etc.) — a real, substantial physics change, not a rounding artifact. **Why nothing caught it for 8 days:** `test_model_source.py::test_mesh_full_matches_twin_masses` exists specifically to compare the two and would have failed instantly, but it `pytest.skip()`s whenever full-mesh STL assets aren't present — true on every GPU pod (where 100% of mesh-family training happens) and, evidently, not reliably true across controller sessions either (the assets are gitignored/regenerated locally). The sibling test that DOES always run, `test_mesh_mjx_twin_builds_and_plants`, only asserted `3.0 < mass < 6.0` — a bound so loose both the correct (3.49) and the corrupted (4.81) value pass it.

**The fix (this cycle):** regenerated `mesh_mujoco/hexapod_mesh_mjx.xml` via the checked-in, unmodified `mesh_mujoco/build_mesh_model.py` (`uv run --with numpy --with scipy --with shapely --with trimesh python build_mesh_model.py`) — confirmed both variants now compile to the IDENTICAL **3.490038 kg** (this also happens to exactly match the pre-`a55173ab` committed values, e.g. chassis 1.472688kg, confirming the fix is a revert-to-intended, not a new choice). Tightened the toothless bound in `test_mesh_mjx_twin_builds_and_plants` to `3.3 < mass < 3.7` (was `3.0..6.0`) with an explicit comment naming why the loose bound is exactly what let this drift for 8 days undetected — re-widen only for a real, deliberate CAD/BOM mass change, and only in the SAME commit as a fresh `build_mesh_model.py` regen. `test_model_source.py` 7/7 + `test_launch_run_model_source_continuity.py` 15/15 green. Only cosmetic collision-geom sign-of-zero deltas otherwise (`git diff mesh_mujoco/`); kinematics/`nq`/`nu`/joint layout untouched (confirmed via `test_mesh_full_matches_twin_masses`'s own `nq`/`nu` check, now passing instead of skip-only).

**Decisive zero-training re-verification (no GPU spend, existing checkpoints, corrected twin only):** re-ran the EXACT flat-pinned probe that closed the whole `over_current` reward-pricing saga, unchanged cfg, only the checked-in twin file fixed underneath:
- `curhot-b23k12` (the ADOPTED champion, already shipped in `todaypolicy-50hz-v1`): was 0/12 `over_current` @ 30.75mm fp @ **4.806kg** (its own PASS verdict); now **0/36** terminations across hold+rise+lower det+sto @ **3.490kg**, rise fp median actually TIGHTER (~26-30mm vs 30.75mm), height fully in-band. Still clean — the fix does not regress the shipped candidate.
- `cont6m2` — the PLAIN PARENT, **BEFORE any of the `k_current_hot`/`k_load_even`/`k_torque_headroom` reward-engineering** (its own original `k_current_hot=1.0`/`current_hot_a=2.0`, the setting every subsequent dose bracket was built to fix): at the buggy 4.806kg mass this checkpoint had 3/12 rise `over_current` (the number that KICKED OFF the whole saga). At the corrected 3.490kg mass, same checkpoint, same cfg, zero retraining: **rise mode is 12/12 clean (0 terminations, det+sto)**; the only termination anywhere in 36 episodes is one unrelated `hold_min_load` in `hold/sto` (1/36). Evidence: `logs/ckpt_eval/massfix_verify/{b23k12,cont6m2}_flatprobe_massfix/report.json`.

**Interpretation, binding:** this is a textbook `sim/objective defect` at the root of the behavior<-incentive<-pricing<-sim chain (RESEARCH_RULES root-cause order) — the 09-11 standwalk saga's own "reward-pricing axis EXHAUSTED, four independent mechanisms, next lever needs a genuinely new structural mechanism" conclusion (immediately above in this file) is **superseded**: the residual it was fighting is very likely this mass bug, not a real actuator/reward-pricing ceiling. This does NOT retroactively invalidate any individual run's own verdict (each was an honest read of what actually happened under the physics that actually existed at the time) but it DOES mean: (a) do not fund any further `k_current_hot`/`k_load_even`/`k_torque_headroom` dose/stack work on this residual — it's very likely already gone; (b) `curhot-b23k12` remains a fine adopted checkpoint (it still passes, even better, post-fix) — no forced re-ship; (c) any FUTURE standwalk/walkcurr/amp/speed GPU launch on the `mesh` default automatically gets the corrected 3.49kg physics with no cfg change (the twin file IS the fix); (d) treat this the way the 08-24 model-family switch was treated for continuity purposes — a resumed/warm-started lineage crossing this fix will experience a real, one-time, ~27% mass drop (lighter, not heavier — same direction as "more headroom," not a new hazard), name it in that run's own hypothesis if the delta matters to the question being asked.

**Blast radius / what's affected:** every mesh-family GPU-trained checkpoint committed/trained between `a55173ab` (2026-09-03) and this fix (2026-09-11) trained under the wrong +37.6% mass — this spans essentially the entire recent campaign: the whole standwalk 50Hz stance saga, the walkcurr widen8/crutchoff seed family + all 50Hz warm-adapt exports, the amp 50Hz retrain wave (`m2plain-scratch`/`discretune`/`styleoff`), and the speed track's stride-geometry frontier. None of this is being blanket-invalidated or force-retrained this cycle (too broad for one pass, and the spot-checks above show the shipped champions hold up fine post-fix) — it is flagged so any future cycle revisiting one of these "closed" sagas checks the corrected-mass premise first, cheaply (a zero-training re-eval like the two above), before assuming a reward/physics ceiling is real. **Also directly relevant to hardware handoff:** every 50Hz bundle already exported to `linux_control/policies/` for Robot Lab (`todaypolicy-50hz-v1`, `bundle_rlonly_v2`) was trained on a robot 37.6% heavier than the real hexapod2 — the mismatch direction (trained-heavier-than-real) means the policy has MORE torque margin on the real robot than it learned to expect, which is the safe direction, not a new hazard, but Robot Lab should have this fact before/alongside any physical trial. Filed `q_20260911T1215Z` in `OPERATOR_QUESTIONS.md` (assume-and-go: fix lands now, no blanket retrain, flag for future re-visits + Robot Lab). **Cross-check, same cycle:** NOT every current/torque-adjacent "closed" residual is this bug — re-ran the walkcurr track's own off-axis-heading `--pinned-heading-panel` reference read under the corrected mass and got the byte-identical front-pair leg-sacrifice signature as every pre-fix read (same sacrificed legs at every off-axis heading, on-axis still clean); that gap is a genuine gait-topology issue, unaffected by mass, and stays closed pending a new structural mechanism (`walkcurr/STATUS.md` 2026-09-11 ~12:2x). Evidence: `git show a55173ab -- .../mesh_mjx.xml`; `mesh_mujoco/build_mesh_model.py` output this cycle; `logs/ckpt_eval/massfix_verify/`; `rl_docs/COMMANDS.md` gotcha #17 (updated, FIXED). **Two more targeted GPU re-checks landed the same cycle-chain, mixed result:** the amp track's FAILed `turnrecipe-scratch-acq15m` (turn+push+fault+style, full stack) repeated verbatim under the corrected mass cuts `over_current` terminations hugely (det 6/6->0/6) but the crouched/static stance and near-zero forward translation persist on video — mass explained the current-rail image, not the missing walk; the already-diagnosed disc-saturation-starves-gait mechanism stands, independent of mass (verdicted FAIL, `amp/STATUS.md` 2026-09-11 ~13:2x). The speed track's `ps200-lift14` stride-geometry frontier, by contrast, DID gain real speed from the fix (+5.8% at the 0.10 pin, stacked with the already-banked free `stance_radius_scale=1.05` dose) — a genuine, if modest and ceiling-shape-unchanged, improvement (verdicted PASS narrow, `speed/STATUS.md` 2026-09-11 ~13:5x). Net pattern so far: the mass bug fully explains a genuine torque/current-pricing residual (standwalk) and partially/superficially explains one (amp turnrecipe's termination rate, not its gait), while leaving a pure gait-topology gap (walkcurr) and a pure kinematic-geometry ceiling (speed's teacher probe) untouched — check the SPECIFIC mechanism a "closed" result rests on before assuming the mass bug is (or isn't) the explanation.

## Stacked `k_load_even=8` + `k_torque_headroom` bracket CLOSED (`le8hr3` FAIL, `le8hr30` FAIL): both flanks converge on the identical 3/24 floor — the reward-pricing axis for the `b23k12` flat-start rise `over_current` residual is now EXHAUSTED across four independent mechanisms (2026-09-11 ~11:4x, standwalk track, triage cycle)

One plain sentence: stacking the two best surviving single-lever PARTIAL doses together makes the residual WORSE, not better, at BOTH tested combinations, closing the whole reward-pricing line of attack on this residual.

`le8hr3` (`k_load_even=8` + `k_torque_headroom=3`) and `le8hr30` (`k_load_even=8` + `k_torque_headroom=30`) — the bottom and top of the headroom bracket, both stacked on the same `loadeven8` dose, both `--init-from-source` off `curhot-b23k12` directly — land at the IDENTICAL pooled gate+owncfg rise `over_current` count: **3/24 each**, worse than either single lever's own 2/24 ceiling (`loadeven8` alone, `headroom3`/`headroom30` alone). `le8hr3`: gate rise/det + rise/sto + owncfg rise/det. `le8hr30`: gate rise/det + owncfg rise/det + owncfg rise/sto — different exact episode indices, same count, same phase (rise-only), same termination reason (`over_current` pinned at the 2.64A rail) both times. Neither run adds a new/worse fall class: both show the identical baseline `tilt_roll` pair (owncfg hold/sto + lower/sto) already present in `b23k12`'s own owncfg baseline and in the standalone `headroom3`/`headroom30` reads. Footprint/height stay comfortably in-band in both (le8hr3 rise fp med 16.15/16.65mm gate/owncfg; le8hr30 rise fp med 5.9mm, height med 1.95mm).

**Binding**: this closes the entire reward-pricing axis for the `b23k12` residual. Four independently-designed mechanisms have now been tried — flat magnitude (`k_current_hot`), spatial load-evenness (`k_load_even`), per-actuator duration debt (`k_torque_headroom`), and the best-dose stack of the latter two across both flanks of the headroom bracket — and every single one plateaus at 2/24 alone or regresses to 3/24 when combined. Two independently-designed doses stacked at OPPOSITE ends of the headroom bracket landing on the exact same worse-than-either-alone floor is strong evidence the 2/24 ceiling is a property of the underlying stall dynamics itself (a single femur-row servo genuinely needs to dwell near the torque rail for part of the flat-start rise, per the 09-11 ~08:0x DIG-IN root-cause), not an artifact of any one reward-pricing axis, dose, or combination thereof. **Do not fund another dose, combination, or stacking variant of `k_current_hot`/`k_load_even`/`k_torque_headroom` on this lineage without new evidence.** The next escalation, if pursued, must be a genuinely new structural mechanism that acts on the ACTION/motion side of the stall (an action-space rate/torque-margin shaping, or a gait-timing change to the rise trajectory itself) rather than another reward-pricing guess — not yet designed, no agent-doable next step identified this cycle beyond flagging it needs fresh first-principles thinking. `curhot-b23k12` (PASS with its documented 1/12 rise `over_current` residual) remains the adopted stand/lower checkpoint, already shipped in `todaypolicy-50hz-v1` with this exact limitation named honestly in its GO_NOGO. Evidence: `ops.sh entry cw-stand50hz-stance-tuckclock-scratch6m-dqfix-retention-s1-exploreresettle-budget6m-cont6m2-curhot-b23k12-le8hr{3,30}`; `logs/ckpt_eval/cw_stand50hz_stance_tuckclock_scratch6m_dqfix_retention_s1_exploreresettle_budget6m_cont6m2_curhot_b23k12_le8hr{3,30}_{gate,owncfg}/report.json`.

## `k_torque_headroom` dose bracket CLOSED: headroom3/headroom30 both PARTIAL at the identical 2/24 ceiling, headroom10 FAILs with a video-confirmed new debt-dodge pathology — THIRD independently-designed structural lever now plateaus at the same floor as `k_load_even` (2026-09-11 ~11:2x, standwalk track, triage cycle)

One plain sentence: the brand-new duration-priced torque-headroom debt mechanism (built this same window to replace the two closed magnitude/spatial levers) does NOT beat `k_load_even`'s own 2/24 ceiling at either flank of its 3-dose bracket, and its middle dose actively teaches the policy a new way to cheat the price instead of paying down the stall.

Pooled gate+owncfg `over_current` (24 slots/arm, same panel as `k_load_even`/`drwiden35`): `headroom3` (k=3) = 2/24, `headroom30` (k=30, 10x stronger) = 2/24 — **identical count AND identical episode locations** (rise/det + rise/sto for over_current; hold/sto + lower/sto for tilt_roll, matching the b23k12 parent's own owncfg baseline exactly). `headroom10` (k=10, the middle dose) = 7/24 — worse than the parent's own 3/24 baseline and worse than both flanks — PLUS a fall class never seen on this lineage: `hold_min_load` (owncfg `hold/sto`). This is not a monotonic-then-overdose curve (contrast `k_load_even`'s clean inverted-U peaking at dose 8): the two flanking doses (3 and 30) land at the exact same floor while only the untested-in-between dose triggers the new pathology. Frame-strip inspection of the failing episode (`hold_sto_5`, `ops.sh frames`) confirms the mechanism directly: the robot is quietly holding station (not rising, not moving) and lifts a leg off the ground late in the episode — the policy learned to dodge the per-actuator torque-headroom debt by unloading, which trips the `hold_min_load` anti-cheese floor instead of paying the price. This is exactly the "if false" risk named in the run's own pre-registered hypothesis (`ops.sh entry ...-headroom10`). Verdicted PARTIAL/FAIL/PARTIAL respectively (`ops.sh entry cw-stand50hz-stance-tuckclock-scratch6m-dqfix-retention-s1-exploreresettle-budget6m-cont6m2-curhot-b23k12-headroom{3,10,30}`).

**Binding for the next read**: three independently-designed structural mechanisms now targeting the exact same b23k12 flat-start rise `over_current` residual — flat magnitude pricing (`k_current_hot`, closed at its own ceiling), spatial load-evenness pricing (`k_load_even`, closed at 2/24 peaking at dose 8), and per-actuator duration/debt pricing (`k_torque_headroom`, closed here at 2/24 at doses 3 and 30) — all plateau at the SAME 2/24-pooled floor and none clears the pre-registered <=1/24 PASS bar alone. This is itself evidence: a single reward-pricing axis, whatever it prices (magnitude, space, or time), caps out at this floor on this lineage. The next escalation launched this same cycle stacks the two SURVIVING best-PARTIAL doses together (`k_load_even=8` + `k_torque_headroom={3,30}`, `...-le8hr3`/`...-le8hr30`, both VERIFIED RUNNING) to test whether combining mechanisms that act on different axes (space vs. time) compounds rather than merely repeats the same floor. If the stack also lands at >=2/24, the floor is likely a property of the underlying stall dynamics rather than the pricing axis, and the next lever needs to leave reward-shaping entirely (e.g. an action-space/gait-timing change, or accepting `curhot-b23k12`'s documented residual as the shipped baseline). `curhot-b23k12` (2M, PASS with its documented 1/12 rise over_current residual) remains the standwalk track's adopted checkpoint and the one already shipped in `todaypolicy-50hz-v1`. Evidence: `ops.sh entry cw-stand50hz-stance-tuckclock-scratch6m-dqfix-retention-s1-exploreresettle-budget6m-cont6m2-curhot-b23k12-headroom{3,10,30}`; `logs/ckpt_eval/cw_stand50hz_stance_tuckclock_scratch6m_dqfix_retention_s1_exploreresettle_budget6m_cont6m2_curhot_b23k12_headroom{3,10,30}_{gate,owncfg}/report.json`.

## `k_load_even` dose bracket CLOSED: loadeven16/32 both FAIL, peak stays at loadeven8 (PARTIAL) — combined with `drwiden35`'s FAIL this same window, BOTH named structural candidates for the b23k12 residual are now capped short of a clean PASS (2026-09-11 ~10:4x, standwalk track, triage cycle)

One plain sentence: escalating the load-evenness dose further past loadeven8 (k_load_even 8->16->32) does NOT keep improving the flat-start over_current stall-fight the way the earlier 2->8 step did — it's an inverted-U, not a monotonic ramp, and dose 8 is the peak.

Rise-mode `over_current` pooled across gate+owncfg (det+sto, n=6/mode, 24 slots): loadeven2=4/24 (FAIL), loadeven8=2/24 (PARTIAL, best so far), **loadeven16=2/24 (unchanged from loadeven8 — the pre-registered gate treats "unchanged" as FAIL, not a pass)**, **loadeven32=3/24 (worse than loadeven8)**. loadeven16 additionally opens a failure location never seen at doses 2/8: `over_current` AND `tilt_roll` both spread into the LOWER mode (`owncfg lower/det/3` over_current, `lower/det/4` tilt_roll) — total tilt_roll count rises to 3 vs loadeven8's 2. loadeven32 stays contained to rise mode but its own over_current count is the worst of the whole family (3/24), even though it buys the tightest footprint/height and fewest tilt-falls (rise fp median 10.35mm, height median 2.55mm, tilt_roll 1/72 — best of the bracket on those axes). Both verdicted FAIL (`ops.sh entry cw-stand50hz-stance-tuckclock-scratch6m-dqfix-retention-s1-exploreresettle-budget6m-cont6m2-curhot-b23k12-loadeven{16,32}`).

**Binding for the next read**: the `k_load_even` lever caps out at dose 8 (PARTIAL, 2/24 over_current, still short of the <=1/24 PASS bar) — do not push this dose further on this lineage; the trade at higher doses (tighter footprint/current-stability elsewhere for MORE rise-mode over_current, or new failure locations) is the signature of an overdose, not an under-tuned setting. Combined with the immediately-preceding `drwiden35` FAIL (DR-widening, the other named structural candidate for this same residual), **both of the standwalk track's pre-registered structural candidates for the b23k12 flat-start-rise residual are now closed short of a clean PASS on their own.** The next lever needs to be a genuinely different structural mechanism — an explicit per-leg torque-margin/headroom reward term (never built), or stacking loadeven8's dose on top of an as-yet-untried axis (e.g. current-hot pricing dose adjustment alongside load-evenness, or a pacing change specifically scoped away from the already-refuted `rise_ramp_s` axis) — not a further dose/DR-scale guess on either tried lever. `curhot-b23k12` (2M, PASS with its documented 1/12 rise over_current residual) remains the standwalk track's adopted checkpoint and the one already shipped in the `todaypolicy-50hz-v1` bundle. Evidence: `ops.sh entry cw-stand50hz-stance-tuckclock-scratch6m-dqfix-retention-s1-exploreresettle-budget6m-cont6m2-curhot-b23k12-loadeven{16,32}`; `logs/ckpt_eval/cw_stand50hz_stance_tuckclock_scratch6m_dqfix_retention_s1_exploreresettle_budget6m_cont6m2_curhot_b23k12_loadeven{16,32}_{gate,owncfg}/report.json`.
...
**The dose bracket's top arm, `curhot-b23k36` (`hot_a=2.3/k=36`, ~4.2/tick at the rail, dominates all rise income), IS a genuine FAIL but for a different reason than first reported.** Its own literal flat-pinned probe (re-run this cycle, train-1): `over_current` is fully suppressed too (0/12, confirming the price mechanism works even harder at this dose) but `footprint_err_end_mm` COLLAPSES to 57.7-67.4mm (median 60.1mm — far worse than the 18.25mm parent and the 35mm bar), `valid_plant` 0/12; height stays in-band (0.3-8.0mm), so this is a footprint-specific overdose (the policy avoids the price by refusing to commit to a tight stance), not the gate's literal height-collapse text but the same overdose family.

**Correction (same cycle-window): the FIRST FAIL verdict on `curhot-b23k36`** (recorded ~08:1x, same window, concurrent cycle) **used the wrong report** — its cited numbers (fp 37.8mm for b23k36, 15.35mm/20.55mm for k12/k6, `valid_plant` 7/12 and "over_current still 1/12 in ALL THREE arms") come from the auto-prestaged MIXED-START `_gate` report (2 genuine `start_kind='flat'` episodes out of 12, diluted by 10 easier rsi/bridge/crouch starts) — the exact previously-documented gotcha this saga already hit once on `fprent-k15` (see the 09-05 ~05:3x RESOLVED entry). That misread led to two further mistaken conclusions: (a) that k12/k6 ALSO still show 1/12 over_current (false — both are 0/12 on the correct probe) and (b) that "the reprice-dose axis alone cannot zero out the stall-fight residual" (false — it does, cleanly, at k12/k6's doses) — which is what triggered a `rise_ramp_s` pacing escalation (`-rampslow8`, launched same window on train-1) on a false premise. Not killing that run (still an independently informative arm on its own pacing hypothesis), but the correct reading of the bracket is: **adopt `curhot-b23k12`** (smallest-collateral passer, best margin of the two PASSes) as the new flat-start-rise recipe for this lineage; k=36 is simply past the dose's usable range, not evidence the mechanism failed. `curhot-b23k36`'s own verdict is corrected to FAIL-for-overdose-collapse (`ops.sh entry ...-curhot-b23k36`, `FORCE=1` correction, this cycle) with the true numbers on record; `curhot-b23k12`/`curhot-k6` are freshly verdicted PASS (`ops.sh entry` for each, this cycle).

**Binding for the next read**: `curhot-b23k12`'s checkpoint is the standwalk track's new leading flat-start-rise candidate — next step is folding it into the broader stage-1 stance panel (mixed-start `pod_eval` gate, own-DR 0.2, per the track's actual DONE-gate machinery) rather than another dose/discovery arm; the flat-pinned-probe-only reads in this whole saga were a narrower diagnostic, not the track's real acceptance gate. Evidence: `ops.sh entry cw-stand50hz-stance-tuckclock-scratch6m-dqfix-retention-s1-exploreresettle-budget6m-cont6m2-curhot-{b23k12,k6,b23k36}`; `logs/ckpt_eval/cw_stand50hz_stance_tuckclock_scratch6m_dqfix_retention_s1_exploreresettle_budget6m_cont6m2_curhot_{b23k12,k6,b23k36}_flatprobe/report.json` (this cycle's on-pod probes, copied back to controller).

## `fprent-k0ctl-gentlestd-budget6m-cont6m` FAIL — the tiny-magnitude (-3.0 override) recipe REGRESSES under extra budget instead of continuing to improve, capping that magnitude-branch at its 8M-step parent; sharpens the magnitude-vs-budget axis into a real tradeoff, not two independent additive levers (2026-09-11 ~07:5x, standwalk track, triage cycle)

One plain sentence: giving the `gentlestd-budget6m` recipe (small `--warm-log-std-override -3.0`, already at 8M steps, fp~47mm PARTIAL) a further +6M continuation (14M total) made the flat-start rise clause WORSE, not better — footprint_err_end_mm rose from the parent's 45.9-51.5mm (median ~47mm) to 53.8-62.0mm (median ~57mm), and ALL 12/12 rise episodes (det+sto) now TERMINATE via `over_current` (roll_class='fell', pinned at the same `cur_max_a=2.64A` ceiling every time) versus ZERO rise terminations in the parent — a new, worse failure signature, not just a missed threshold. Ran this run's own pre-registered flat-pinned probe on-pod (train-0, DR-0, det+sto n=6/mode, `mesh_mjx_twin` matching training physics). Hold/lower stayed fully clean (zero termination, herr<=1.7mm) — the regression is rise-only. Training reward kept rising in aggregate (quarters 17.8/348.2/613.6/658.0) but the rise-specific `env/rise_score` metric itself FELL over the same window (0.82->0.757) — the extra steps bought something other than better rise behavior; checked against the gate's own explicit pre-registered PARTIAL/FAIL branches (not an auto-continue) and the fp-regression + new-terminations pair unambiguously matches the FAIL branch. Verdicted FAIL (`ops.sh entry cw-stand50hz-stance-tuckclock-scratch6m-dqfix-retention-s1-fprent-k0ctl-gentlestd-budget6m-cont6m`).

**Why this matters — corrects the magnitude-vs-budget reading**: this is the sharpest evidence yet that magnitude and budget are NOT simply additive/independent levers (an earlier entry's tentative reading). The tiny -3.0-override branch (`gentlestd`->`gentlestd-budget6m`->`-cont6m`) plateaus fast and then REGRESSES with more budget (61mm->47mm->57mm over 2M->8M->14M), while the larger -1.0-override, no-magnitude-fix branch (`exploreresettle-budget6m`->`-cont6m`->`-cont6m2`) keeps monotonically improving over an equivalent budget extension (99mm->83mm->38.8mm->18.25mm over 0->6M->12M->18M). Smaller re-exploration noise converges to a better-looking checkpoint sooner but exhausts its useful search fast and can actively overfit/regress past that point; larger noise takes longer per step but keeps paying off. **Binding for the next read**: do not fund a 2nd continuation of the tiny-magnitude recipe — `gentlestd-budget6m` (8M steps, fp~47mm, PARTIAL) is now this magnitude-branch's FINAL adoptable checkpoint; route all further footprint-saga budget at the -1.0-override budget-only line instead, which is not yet plateaued (see `exploreresettle-budget6m-cont6m2` entry above/below). The shared residual across BOTH lines — `over_current` pinned at `cur_max_a=2.64A` on flat-start rise — remains DIG-IN flagged (current/effort-shaping reward-or-pacing mechanism design, not a guessed cfg value); this run adds convergent evidence for that same design need, it does not resolve it. Evidence: `ops.sh entry cw-stand50hz-stance-tuckclock-scratch6m-dqfix-retention-s1-fprent-k0ctl-gentlestd-budget6m-cont6m`; `logs/ckpt_eval/cw_stand50hz_stance_tuckclock_scratch6m_dqfix_retention_s1_fprent_k0ctl_gentlestd_budget6m_cont6m_flatprobe_{det,sto}/report.json` (this cycle's on-pod probe).

## `exploreresettle-budget6m-cont6m2`: footprint lever now basically SOLVED (fp 18.25mm, deep under the 35mm bar) but valid_plant is STATIC 9/12 across two straight +6M continuations — a different, current-limited residual, not a footprint one (2026-09-11 ~07:3x, standwalk track, triage cycle)

One plain sentence: giving the same budget-only recipe a SECOND +6M continuation (12M->18M total) closed footprint almost all the way (38.8mm->18.25mm median, now less than half the 35mm bar) and tightened height further (0.2-9.2mm->0.1-4.5mm), but valid_plant did not move AT ALL — 9/12 both times, with the identical det4/6+sto5/6 split — because the blocker is now 3 `over_current` terminations on flat-start rise, each pinned at the exact same `cur_max_a=2.64A` servo-current ceiling, plus a new `hold_min_load` fall in hold/sto (roll_class='fell'). Ran this run's own pre-registered flat-pinned probe on-pod (train-1, DR-0, det+sto n=6/mode, `mesh_mjx_twin` — the auto-prestaged mixed-start `_gate`/`_owncfg` reports do NOT match this run's literal-flat criterion and were not used). Verdicted PARTIAL (`ops.sh entry cw-stand50hz-stance-tuckclock-scratch6m-dqfix-retention-s1-exploreresettle-budget6m-cont6m2`). Why this matters: this decisively separates the two things this saga has been conflating — footprint-tightness and valid_plant — showing budget-only training keeps paying off on the former while doing literally zero net work on the latter over an identical 6M-step dose; a 3rd continuation of the exact same recipe is not expected to move the over-current axis at all (nothing did across the first repeat). Convergent evidence: the parallel magnitude+budget line (`fprent-k0ctl-gentlestd-budget6m`) independently hit its OWN new hold-side termination right as its footprint closed to ~47mm — both budget-driven lines are trading footprint gains against a servo-load/hold-stability margin, not a coincidence of one recipe. **Binding for the next read**: do NOT launch a further blind budget continuation on either line to chase valid_plant — per this track's own precedent for an analogous current-adjacent residual ("next escalation is a repriced footprint/current reward term — DIG-IN flagged, not self-adjudicated"), this needs a current/effort-shaping mechanism design (current_hot_a dose, rise_ramp_s pacing, or an explicit torque-headroom term for the flat-start rise) — DIG-IN flagged, not a guessed cfg value. Evidence: `ops.sh entry cw-stand50hz-stance-tuckclock-scratch6m-dqfix-retention-s1-exploreresettle-budget6m-cont6m2`; `logs/ckpt_eval/cw_stand50hz_stance_tuckclock_scratch6m_dqfix_retention_s1_exploreresettle_budget6m_cont6m2_flatprobe/report.json` (this cycle's on-pod re-run, copied back to controller).

## NEW EVAL-METHODOLOGY BUG: controller-local `eval_checkpoint` silently evaluates a DIFFERENT (37.5% lighter) model than every GPU-pod training/eval run (2026-09-11 ~07:1x, standwalk track, triage cycle)

One plain sentence: running `eval_checkpoint` directly on the controller (not via `kubectl exec`/`ops.sh podeval`) with the default `env.model_source=mesh` silently loads `model_variant=full_mesh` (3.494kg — the gitignored real-CAD STL assets the controller happens to have checked out) instead of the `mesh_mjx_twin` (4.806kg) every GPU pod uses for BOTH training and eval, even though `servo_model.py`'s own docstring documents the two as having "same kinematics, masses and inertia" — an empirically false claim (+37.5% mass delta, unaudited/unexplained).

Found while triaging `cw-stand50hz-stance-tuckclock-scratch6m-dqfix-retention-s1-fprent-k0ctl-gentlestd`: a first flat-pinned probe run locally on the controller read `footprint_err_end_mm`≈23mm (would have been an easy PASS) on the SAME checkpoint/seed/cfg that a `--cfg-set env.model_source=mesh_mjx`-forced re-run (matching training physics) read at ≈61mm (a clear, if improved-over-parent, FAIL) — discovered via a `model_variant` field mismatch in the two `report.json`s before any verdict was written, so no bad verdict shipped, but this is a real trap: any controller-run custom eval of a pod-trained mesh-family checkpoint that skips this override is silently off the training distribution. Filed as `rl_docs/COMMANDS.md` gotcha #17 (binding: always add `--cfg-set env.model_source=mesh_mjx` to controller-run custom evals of pod-trained checkpoints, or just run them on-pod per the standing convention). The root mass-audit discrepancy between `full_mesh` and `mesh_mjx_twin` is NOT fixed here — it's a separate, still-open bug for whoever next touches `mesh_mujoco/build_mesh_model.py` or the mjx-twin XML generator; this entry only documents the trap and the workaround. Evidence: `ops.sh entry cw-stand50hz-stance-tuckclock-scratch6m-dqfix-retention-s1-fprent-k0ctl-gentlestd(-budget6m)` (verdict text); `rl_docs/COMMANDS.md` #17; `rl_move/sim/servo_model.py` `resolve_model_source`/`_mesh_xml_text` docstrings.

## `exploreresettle-budget6m-cont6m` (budget-only, NO magnitude override) is now the closest result in the WHOLE footprint saga: fp median 38.8mm, height fully clean — one step from the 35mm bar (2026-09-11 ~06:5x, standwalk track, triage cycle)

One plain sentence: pure budget (no magnitude lever at all, `k_rise_footprint_pen=0.0`, override left at the standard -1.0) taken to 12M total steps (6M->12M continuation) landed rise footprint_err_end_mm 12-episode combined median **38.8mm** (36.3-41.7mm, one 73.9mm outlier) and height_err_end_mm **0.2-9.2mm across all 24 hold/rise/lower episodes** — fully clean, matching/beating gentlestd's height result — with hold/lower zero-termination throughout. valid_plant 9/12 (need >=10/12), so still short, but this is closer to the 35mm bar than `gentlestd`'s magnitude-only fp~61mm or `budget6m`'s first-6M fp~83mm. Notably the footprint gap closed FASTER in the second 6M (81.7-85.5mm->38.8mm, ~44mm) than the first 6M did (99mm->83mm, ~16mm) even though `rollout/ep_rew_mean` itself had already flattened/gone noisy (530-660 range) over that window — the footprint metric kept refining on a reward plateau. Verdicted PARTIAL/CONTINUE (`ops.sh entry cw-stand50hz-stance-tuckclock-scratch6m-dqfix-retention-s1-exploreresettle-budget6m-cont6m`); **`-cont6m2` (12M->18M total, same recipe, no lever changed) queued to backlog this cycle** to test whether the accelerating trend actually crosses 35mm/10-12 or plateaus just short. Corrected reading of the magnitude-vs-budget axis: budget ALONE, given enough of it, may be sufficient without any override-magnitude fix at all — the two confirmed levers (gentlestd's magnitude, budget6m's budget) may not need combining; watch `gentlestd-budget6m` (magnitude+budget together, still training on train-0) as the natural comparison once it lands.

One plain sentence: the entry below guessed "magnitude alone doesn't help" from `gentleanneal`'s FAIL (override -1.0->-2.0 + anneal-frac 0.5->1.0), but the CLEANER single-lever magnitude test — `fprent-k0ctl-gentlestd` (override -1.0->-3.0 only, the launcher's own standard acquisition-phase value, same anneal-frac 0.5, same 2M budget) — reads BETTER than either sibling: rise flat-pinned probe (DR-0, det+sto n=6/mode) footprint_err_end_mm median **~61.1mm** (60.2-63.6mm) vs k0ctl's ~99mm and `budget6m`'s 81.7-85.5mm, AND height_err_end_mm is **2.9-6.8mm across all 12 rise episodes — the first fully height-clean result anywhere in this saga** (budget6m instead let height regress to 18-25mm). Hold/lower clean both. Still short of the 35mm/valid_plant>=10/12 pass bar (0/12), verdicted PARTIAL (`ops.sh entry cw-stand50hz-stance-tuckclock-scratch6m-dqfix-retention-s1-fprent-k0ctl-gentlestd`). Corrected reading of the whole axis: magnitude and budget are BOTH real, non-redundant, partial levers (gentlestd's clean magnitude fix beats budget6m's noisy-magnitude-plus-budget on both footprint AND height) — `gentleanneal`'s failure was the anneal-frac/magnitude-pair it used, not evidence against magnitude generally. **Follow-up launched this cycle**: `cw-stand50hz-stance-tuckclock-scratch6m-dqfix-retention-s1-fprent-k0ctl-gentlestd-budget6m` (respec of gentlestd, steps 2M->6M via `--phase acquisition`, same -3.0 override) — combines the two independently-confirmed levers to test whether they're additive enough to clear 35mm with height still clean. VERIFIED RUNNING train-0. If it FAILS (fp stays ~61mm, no added benefit from budget on top of magnitude), the recipe family caps at PARTIAL and the next candidate is structural (never re-floor log_std at all on an already-converged checkpoint, only anneal down from the parent's own converged value), not a 4th magnitude/budget permutation.

## RESULT of the exploreresettle 2-arm batch: BUDGET is a real (if incomplete) lever, exploration MAGNITUDE/schedule alone is not — a +6M-step continuation launched (2026-09-11 ~06:2x, standwalk track, same triage cycle)

One plain sentence: tripling the fine-tune budget (2M->6M, same `--warm-log-std-override -1.0` as the k0ctl control) genuinely tightened the flat-start footprint from k0ctl's ~99mm to **81.7-85.5mm** (~30% of the gap back toward the pre-explore-restore 46-48mm floor, the tightest of any explore-restore arm in this whole DIG-IN) — but softening the override magnitude instead (`-exploreresettle-gentleanneal`: override -1.0->-2.0, anneal-frac 0.5->1.0, same 2M budget) did NOT help at all (fp 103.4-108.9mm, if anything slightly worse than k0ctl). Neither arm clears the 35mm gate bar and `budget6m`'s height_err_end_mm regressed to 18.4-25.4mm (worse than k0ctl's 12.7-16.1mm) — a real tradeoff, not a clean win. Both flat-pinned-probed DR-0 det+sto n=6/mode; hold/lower clean in both (zero termination). `budget6m`'s own training reward was still rising every quarter (32.6/301.9/542.9/620.1) at the 6M cutoff — per the 08-21 ruling this licenses a continuation, not a flat verdict, so it was verdicted **PARTIAL - CONTINUE** (not FAIL) and a plain `--init-from-source` +6M-step continuation (`-budget6m-cont6m`, no lever changed) was launched this cycle, VERIFIED RUNNING train-1, to see whether the ~16mm/6M-step improvement trend continues toward 35mm or plateaus. `gentleanneal` was verdicted FAIL (matches its own pre-registered branch cleanly). A concurrent cycle independently tested the same magnitude-axis hypothesis as a single, cleaner lever (`fprent-k0ctl-gentlestd`, override=-3.0 alone) — read both together for the fullest picture of that axis; the direction (magnitude alone doesn't help, budget does) agrees across both.

**Binding for the next read**: if `budget6m-cont6m` (12M steps total) keeps improving fp at a similar per-step rate, fund another continuation — budget is the confirmed lever. If it plateaus around 65-85mm without reaching 35mm, budget alone is insufficient and the next candidate is a genuinely different training-diet change (e.g. a flat-start-only fine-tune instead of this recipe's mixed `rise_rsi_frac=0.5` rsi/crouch/flat goal-mix, since the mixed diet spends most of the budget on easier starts that don't stress the flat-start footprint at all) — not a further budget or anneal-magnitude guess. Evidence: `ops.sh entry cw-stand50hz-stance-tuckclock-scratch6m-dqfix-retention-s1-exploreresettle-{budget6m,gentleanneal}` (verdicts, this cycle); `logs/ckpt_eval/cw_stand50hz_stance_tuckclock_scratch6m_dqfix_retention_s1_exploreresettle_{budget6m,gentleanneal}_flatprobe_{det,sto}/report.json`.

## FOLLOW-UP LAUNCHED on the RESOLVED entry below: two single-lever DIG-IN candidates for the explore-restore protocol itself, both from the rent-OFF k0ctl recipe (2026-09-11 ~05:4x, standwalk track, same triage cycle that closed the fprent grid)

One plain sentence: since the fprent grid decisively showed the ~90-120mm drift + height co-failure is the explore-restore PROTOCOL (not the reward), this cycle launched the RESOLVED entry's own two named next candidates as a clean 2-arm batch, each changing exactly one lever off the `k0ctl` attribution-control recipe (rent OFF, legacy reward, otherwise bit-exact): `cw-stand50hz-stance-tuckclock-scratch6m-dqfix-retention-s1-exploreresettle-budget6m` (same `--warm-log-std-override -1.0`/`--log-std-anneal-frac 0.5`, steps 2M->6M — tests whether the freshly-unfrozen policy just needed more time, since `env/rise_score` was still moving at every 2M-cutoff in the fprent grid) and `-exploreresettle-gentleanneal` (steps stay 2M, but `--warm-log-std-override -1.0->-2.0` + `--log-std-anneal-frac 0.5->1.0` — a smaller, slower re-injected perturbation instead of a longer one). Both `respec --from fprent-k0ctl` (so `--init-from` still points at the untouched `dqfix-retention-s1` parent, not k0ctl's own post-drift checkpoint), `--phase acquisition` with `--evidence` naming the k0ctl canary (reward/rise_score still moving at its own 2M cutoff). VERIFIED RUNNING: `-budget6m` on train-1, `-gentleanneal` on train-2. Gate for each: flat-pinned probe, PASS needs rise valid_plant>=10/12 + fp median<=35mm + hold/lower clean; if BOTH fail, the explore-restore protocol is unworkable on this lineage at any tested setting and the next escalation is a from-scratch retrain or a different transfer mechanism entirely, not a third anneal/budget guess. Evidence: `ops.sh entry` for both runs (launch args); RESOLVED entry immediately below.

## RESOLVED (closes the ~04:2x INTERIM entry below): the fprent 4-arm grid (k0ctl/k5/k15/k45) all converge to the SAME ~90-120mm footprint drift + a new height co-failure — this is the EXPLORE-RESTORE PROTOCOL destabilizing the lineage, not the rent term; footprint_rent_m() is refuted as a fix for this symptom (2026-09-11 ~05:3x, standwalk track, triage cycle)

One plain sentence: the k0ctl attribution control (rent completely disabled, k=0, reward otherwise bit-exact legacy) drifts to the SAME ~90-120mm flat-start footprint band and the SAME new height-undershoot co-failure as every rent dose tested (k5/k15/k45), which is exactly the gate's own pre-registered "if k0ctl shows the same height undershoot, it's the protocol, not the rent term" branch — so the standalone rent mechanism (`footprint_rent_m()`) is refuted as a lever for this defect; the real cause is restoring genuine PPO exploration (`--warm-log-std-override -1.0`) into an already-converged checkpoint under a short (2M-step) fine-tune budget.

**Data-quality finding first**: this cycle discovered the k15 and k0ctl flat-pinned-probe artifacts named in the original ask were CORRUPTED — run under a stale/wrong command that never overrode `control.hz` (ran at the champion's 100Hz instead of this lineage's trained 50Hz), left `--dr-scale` at the 0.2 default instead of 0, used the wrong `--modes` (`hold track rise`, missing `lower`, with a spurious `track` mode), and — because the same broken command was invoked twice with no `--stochastic` difference actually taking effect — wrote byte-identical `report.json` files to both the `_det` and `_sto` output directories. Root-caused via each pod's own `/tmp/*.log` (`[motor-contract] ... @ 100 Hz` where 50 Hz was expected). Fixed by copying the correct template (verified against the working `k5`/`k45` arms on train-4, which DID pass `--cfg-set control.hz=50 --dr-scale 0.0 --modes hold rise lower`) and relaunching both k15 and k0ctl on train-0 (`*_flatprobe2_{det,sto}`, this cycle). A concurrent cycle independently found and re-ran the same k15 bug in parallel (`*_flatprobe_{det,sto}_FIXED`) — expected duplication under the concurrency model, not a conflict; both should agree.

**Corrected 4-arm comparison** (all flat-pinned: `goal.rise_flat_frac=1.0/partial=0/rsi=0`, DR-0, det+sto n=6/mode):
- `k0ctl` (rent k=0, **control**): fp 97.6-103.2mm (med ~99mm); height_err_end_mm 12.7-16.1mm, `plant_fail` includes `'height'` in 11/12 episodes; **zero terminations**.
- `k5` (low dose): fp 108.3-119.9mm (med ~112mm); height 18.0-30.2mm, height-fails 9/12; zero terminations; rise return -305 to -392.
- `k15` (mid dose, corrected): fp 88.4-113mm; height 5.0-11.8mm (height-clean, no `'height'` in plant_fail); **5/12 real `over_current` terminations** — the only arm with actual falls; rise return -811 to -1344.
- `k45` (high dose): fp 91.8-101.0mm (med ~95mm); height 6.4-13.8mm (height-clean); zero terminations; rise return -2025 to -2409 (a ~6x larger debit than k5 for a 9x larger k and only ~15% better fp — the tax scales with k, the behavior barely does).

None of the three rent doses land below the established no-rent band (66-107mm, ~15 prior arms) or anywhere near the gate's 35mm bar; all four arms (including rent-OFF) sit in the same ~90-120mm neighborhood. Hold/lower are clean and unaffected in all four (hold fp 23-34mm/herr<=2.2mm, lower herr 0.4-4.3mm, zero terminations in hold/lower every arm) — confirms the regression/protocol issue is rise-only.

**Binding conclusion**: retire `footprint_rent_m()` (and the fade-reprice mechanism before it) as levers for the flat-start footprint drift on THIS lineage under a 2M-step explore-restore fine-tune — three independent reward-shape treatments (legacy fade, repriced fade, standalone rent at 3 doses) all converge to statistically the same wrong footprint, which is the signature of a training-dynamics artifact, not a reward-design one. The two live next candidates, per the gate's own pre-registered text, are (a) more fine-tune budget (let the re-exploring policy resettle fully — none of these arms fully re-converged, `env/rise_score` was still moving at the 2M cutoff in every arm) or (b) a gentler log-std re-anneal (smaller `--warm-log-std-override` magnitude or longer `--log-std-anneal-frac`) so real exploration doesn't fully re-destabilize an already-good basin. Verdicts recorded: `ops.sh entry cw-stand50hz-stance-tuckclock-scratch6m-dqfix-retention-s1-fprent-{k0ctl,k5,k45}` (k15 left to its owning concurrent cycle). Evidence: `/tmp/fprent_pull/*_report.json` (k5/k45, pulled from train-4); `/tmp/fprent_pull2/*_report.json` (k15/k0ctl corrected re-run, pulled from train-0); `logs/experiments/cw-stand50hz-stance-tuckclock-scratch6m-dqfix-retention-s1-fprent-{k5,k45}/wandb_history.csv`.

**Follow-up launched this cycle (assume-and-go, single lever)**: `cw-stand50hz-stance-tuckclock-scratch6m-dqfix-retention-s1-fprent-k0ctl-gentlestd` (respec of k0ctl, VERIFIED RUNNING train-0) — identical recipe, only `--warm-log-std-override` changed from `-1.0` (std=0.368rad, ~21deg — far noisier than the launcher's own `-3.0` acquisition-phase default) to `-3.0` (std=0.0498rad, ~2.85deg). Tests candidate (b) directly: is the override MAGNITUDE (not the reward shape) the causal lever for the ~90-120mm drift + height co-failure seen identically in all four grid arms including rent-OFF. Own gate registered in the ledger (adopt if fp recovers toward the pre-explore-restore 46-48mm band and the height co-failure resolves; refutes the magnitude hypothesis if it doesn't, pointing to budget/anneal-frac next). Picking the override-magnitude lever over a budget increase first because it is the more surprising/informative single change (7.4x noise delta) and the gate reads the sharpest test of "is any recovery possible at 2M steps" before spending more steps on a still-unproven mechanism.

## INTERIM ENTRY THIS SUPERSEDES (2026-09-11 ~04:2x, standwalk track, triage cycle) — kept for the record, not binding

One plain sentence: the first real test of the DIG-IN-requested standalone footprint "rent" term (`footprint_rent_m()`, additive, non-saturating, outside `plant_f` — built the prior cycle, wired opt-in via `reward.k_rise_footprint_pen`) shows a LOW dose (k=5) making the rise clause fail WORSE and in a NEW way, not better: `cw-stand50hz-stance-tuckclock-scratch6m-dqfix-retention-s1-fprent-k5`'s det flat-probe reads rise `footprint_err_end_mm` = **108.3-118.7mm** (worse than this base's own no-rent band, 66-107mm across the whole saga) AND, for the first time in this entire footprint saga, `plant_fail` includes **`'height'`** (height_err_end_mm 19.2-28.7mm — the robot is no longer even reaching the commanded height) plus `current` on 3/6 episodes. Hold and lower are UNAFFECTED and clean (hold fp=23.2mm/herr=0.2mm all 6, lower herr 0.9-2.7mm all 6) — the regression is rise-only, consistent with the rent term's scoping, but the direction is backwards from the hypothesis and the height co-failure is a mechanism never seen in ~15 prior arms of this saga (which always converged on footprint-only misses with height/current otherwise clean).

This was left unverdicted pending the full 4-arm grid; see the RESOLVED entry immediately above for the completed adjudication.

## CLOSED: the footprint-reprice-via-fade-shape saga is dead — all 3/3 exploration-restored arms STILL fail, one of them WORSE (a real fall) than its frozen-exploration twin (2026-09-11 ~03:4x, standwalk track, triage cycle)

One plain sentence: restoring genuine PPO exploration noise (`--warm-log-std-override -1.0`, confirmed via `log_std_anneal/all/value` reading -1 at step 0 and re-annealing to -4.0, not the prior grids' accidentally-frozen -4.0-from-tick-0 bug) did NOT let any of the 3 independent warm-start bases find a better footprint optimum under the widened `full_mm=12/zero_mm=100` fade — `clipsched-fpwiden-explore` fp=103-107mm (WORSE than its own frozen-exploration twin's 82.9-85.0mm, and this time with actual `over_current` falls, `roll_class='fell'`, in all 12/12 episodes, not just a quiet wide stance), `clipsched-s1-fpwiden-explore` fp=90-107mm (same band as its 86.8-91.1mm twin), `retention-s1-fpwiden-explore` fp=87-93mm (same band as its 77.7-81.1mm twin, clean, no falls) — vs FAIL.

This decisively eliminates "frozen exploration" as the confound the top-of-file ROOT CAUSE entry (below) proposed: with real search budget restored and confirmed on each run's own `wandb_history.csv`, PPO still cannot find a fp<=40mm optimum under any fade shape tried so far (hard-cliff 25/40, widened-gradient 12/100), across 4 total reward-shape configs x up to 3 seeds each, all converging to the same 66-107mm band. The mechanism itself — a linear fade multiplying into `plant_f` — is refuted as a lever for this defect, not just under-tuned. Hold/lower stay clean and unaffected in all three arms (zero terminations, herr 0.1-5.8mm), so this is purely a rise-clause reward-design problem.

**Binding for the next attempt**: do not try a 5th set of fade endpoints. The next candidate is a genuinely different reward mechanism — e.g. a standalone anchor-distance penalty term applied OUTSIDE the multiplicative `plant_f` product (so it cannot be “paid off” by other reward streams the way a gated multiplier can), or an IK/geometry-level constraint on the rise reference/action space that makes a wide footprint kinematically costlier rather than just reward-costlier. Flagging DIG-IN for a deep-model cycle to design and build this (real reward/env-code design work, not a parameter guess) rather than guessing a 5th reward-shape config. Evidence: `ops.sh entry` for all 3 `-fpwiden-explore` arms (FAIL verdicts, this cycle); `logs/experiments/cw-stand50hz-stance-tuckclock-scratch6m-dqfix-{clipsched,clipsched-s1,retention-s1}-fpwiden-explore/wandb_history.csv` (`log_std_anneal/all/value`); on-pod `logs/ckpt_eval/cw_stand50hz_stance_tuckclock_scratch6m_dqfix_{clipsched,clipsched_s1,retention_s1}_fpwiden_explore_flatprobe_{det,sto}/report.json`.

## ROOT CAUSE CORRECTED: neither footprint-fade reprice attempt could have worked — warm-started fine-tunes inherit the PARENT's already-floored exploration noise, so 4M combined training-steps of reward reshaping ran with ~1 degree of action noise (2026-09-11 ~03:2x, standwalk track, refill cycle)

One plain sentence: both the tightened-cliff reprice (full=25/zero=40) AND the widened-gradient reprice (full=12/zero=100) converged to nearly the SAME wrong ~77-91mm footprint splay regardless of which reward shape they used, because neither fine-tune ever had enough exploration noise left to search for the reward's new optimum in the first place — the fade-shape question these two grids were built to answer is still genuinely open.

Ran the pre-registered widened-fade 3-arm grid (`reward.rise_footprint_full_mm=12`/`rise_footprint_zero_mm=100`, same 3 base checkpoints as the fpreprice grid, `-fpwiden`/`-fpwide` names — see the CLOSED entry below for that grid's own launch). All 3 finished fast (2M steps, ~10min) and FAILED identically to the first grid: `clipsched-fpwiden` fp=82.9-85.0mm, `clipsched-s1-fpwide` fp=86.8-91.1mm, `retention-s1-fpwide` fp=77.7-81.1mm — all in the SAME ~77-91mm band the tighter-cliff reprice produced (76-95mm), despite the widened fade giving a real nonzero gradient at these fp values (e.g. at fp=83mm, `fp_f=(100-83)/(100-12)=0.19`, nonzero, vs the old reprice's hard 0 past 40mm). Hold/lower stayed clean in all three (zero terminations, herr <=5.7mm).

**Actual mechanism, found by reading `train_ppo_mjx.py` and pulling `wandb_history.csv` for both grids**: `log_std_anneal/all/value` reads **-3.999995 at STEP 0** of every one of these fine-tunes (both the fpreprice grid and the fpwiden grid) and stays pinned at -4.0 (std=0.0183 rad, ~1 degree) for the entire 2M-step run — the SAME already-fully-annealed floor the base checkpoint converged to during its own original 6M-step training. Per `train_ppo_mjx.py`'s own `--warm-log-std-override` flag docstring (~line 1868, already built for exactly this class of problem): "`--log-std-init` only affects from-scratch/transplant builds, never a plain warm start" — `--init-from` loads the checkpoint's OWN log_std parameter verbatim, and neither reprice grid passed `--warm-log-std-override`, so every one of these fine-tunes trained with essentially zero exploration noise from tick 0. A near-deterministic policy cannot discover a new, better local optimum in reward-space no matter how favorable the new reward landscape is — this fully explains why two very different fade shapes produced statistically the same (wrong) answer.

Also eliminated a competing hypothesis this cycle: replayed `rise_ref_mesh_scripted.npz` (the rise reference trajectory `k_rise_ref_track` tracks) through `fk_all_feet` against the plant anchors — its OWN final frame converges to `curl_dist=0.76mm`, essentially on-anchor. The reference is not pulling the policy wide; it is a red herring, ruled out directly rather than assumed.

**Binding for the next attempt**: repeat a footprint-fade cfg (the widened full=12/zero=100 band is a reasonable choice, or re-test full=25/zero=40 — the shape question is unresolved either way) but ADD `--warm-log-std-override -1.0` (real exploration, matching the trainer's own from-scratch default) with the existing `--log-std-final -4.0 --log-std-anneal-frac 0.5` re-annealing it back down over the run, and extend the budget (e.g. 4M vs 2M steps) so the freshly-unfrozen policy has time to both find and re-settle into the improved region. This is the first arm in this whole footprint saga that will actually TEST the reward-shape hypothesis rather than repeat the frozen-exploration mistake — launched this cycle (see STATUS.md). If it STILL doesn't move fp_mm, that decisively rules out reward-shape-with-real-exploration too, and the next escalation is a genuinely different mechanism (explicit standalone anchor-distance penalty outside the multiplicative plant_f product, or an IK/geometry-level constraint). Evidence: `ops.sh entry` for all 3 fpwiden/fpwide runs (verdicts, this cycle); `logs/experiments/cw-stand50hz-stance-tuckclock-scratch6m-dqfix-clipsched-{fpreprice,fpwiden}/wandb_history.csv` (`log_std_anneal/all/value`); `train_ppo_mjx.py` `--warm-log-std-override` (~line 1868); rise-reference FK replay (ad-hoc, this cycle, not yet a checked-in script).

## CLOSED: all 3/3 fpreprice arms confirm the SAME regression from three independent warm-start bases — the hard-zero-past-40mm design is refuted, not a seed fluke (2026-09-11 ~03:0x, standwalk track)

One plain sentence: `clipsched-fpreprice`, `clipsched-s1-fpreprice` and `retention-s1-fpreprice` (three different warm-start histories, one of them never having crashed at all) ALL made the flat-start footprint splay worse under the identical `full_mm=25/zero_mm=40` reprice — 76-95mm, 81-85mm, and 67-71mm respectively, vs each checkpoint's own pre-reprice 46-48mm — so this is a property of the mechanism, not one bad seed. All three: FAIL, hold/lower unaffected and clean. See the entry immediately below for the root-cause reasoning (dead zero-income zone past the cliff), which stands as the binding diagnosis. **Next candidate, not yet built or launched:** widen `rise_footprint_zero_mm` well past the observed drift ceiling (~90-100mm) with a small `full_mm` (~10-15mm), so a continuous downward-sloping gradient exists across the whole range the policy actually visits instead of flatlining exactly at the 40mm gate — this preserves the "tighter than legacy everywhere" intent without repeating the dead-zone mistake. Evidence: `ops.sh entry` for all three runs (FAIL verdicts, this cycle); CURRENT_TRUTHS entry below (root-cause).

## FIRST REPRICE ATTEMPT MADE THE FOOTPRINT SPLAY *WORSE*, NOT BETTER — the fix needs a continuous gradient across the observed drift range, not a hard cliff at the gate (2026-09-11 ~02:5x, standwalk track, idle-kick cycle)

One plain sentence: tightening the footprint reward so it pays nothing at all past 40mm (matching the eval cliff exactly) removed the only gradient that had been nudging the policy back toward the anchors, so a 2M-step warm-start fine-tune under the new pricing drifted the flat-start footprint from 46-48mm to **76-81mm** — worse, not better.

Built `footprint_fade(fp_mm, full_mm, zero_mm)` (`rl_move/sim/sim_env.py`, pulled the inline formula into a pure/testable function; new cfg `reward.rise_footprint_full_mm`/`rise_footprint_zero_mm`, both default to the legacy 40/80 values so behavior is bit-exact when unset; 19 new mechanics tests in `rl_move/tests/test_rise_footprint_fade.py`, full non-slow suite green modulo 7 PRE-EXISTING order-dependent failures confirmed identical on unmodified `main` before this diff — see snapshot `exp/stand50hz-footprint-fade-reprice`). Per the SHARPENING entry below, launched a 3-arm warm-start fine-tune grid (`--init-from-source`, 2M steps, `reward.rise_footprint_full_mm=25`/`rise_footprint_zero_mm=40`) from the three checkpoints that entry names as converged on the identical 46-48mm defect: `dqfix-clipsched`, `-clipsched-s1`, `-dqfix-retention-s1`, suffixed `-fpreprice`.

The first arm to finish (`cw-stand50hz-stance-tuckclock-scratch6m-dqfix-clipsched-fpreprice`) was probed on-pod (train-1, DR-0, flat-pinned, det n=6/mode; sto pending at write time). Hold and lower are unaffected and stay clean (hold 6/6 valid_plant, fp=12.2mm, herr 0.7mm; lower herr 0.3-2.1mm, zero terminations) — the reprice did not touch those clauses' behavior. Rise is the surprise: `plant_fail=['footprint']` 6/6, and `footprint_err_end_mm` reads **76.4-81.5mm**, roughly DOUBLE the pre-reprice 46-48mm this exact lineage converged on, even though training's own online `env/rise_score` metric rose smoothly the whole 2M steps (0.08->~0.5, mixed rsi/bridge/crouch/flat start distribution) — a 08-21-ruling-style disagreement between an improving aggregate training signal and a worse held-out flat-start probe, not noise (n=6, zero variance across episodes: 76.4-81.5mm band, all six terminated cleanly with plant_fail=['footprint'] only).

**Root-cause hypothesis** (reasoned from the reward code, not yet confirmed against the sibling arms): the OLD fade (full-pay<=40, fading 40->80) still gave a nonzero, monotonically-decreasing `plant_f`/`rise_score` gradient everywhere from 0 to 80mm — including the ENTIRE range a flat/cold start actually visits while narrowing its footprint over the ramp. The NEW fade (full-pay<=25, zero>=40) makes `rise_score` IDENTICALLY ZERO for anything from 40mm to infinity: a converged policy already sitting at 47mm-plus, with `log_std` pinned near its floor (near-deterministic, per the same instability history documented below), has almost no exploration noise left to sample back down into the <=40mm region where the only nonzero reward now lives, so PPO has no local signal telling it "47mm is better than 80mm" anymore — only "either one scores zero." Other live reward streams (effort/current, stability/termination avoidance) plausibly still reward the WIDER stance's extra CoM margin with nothing left in this term to counter it, so the policy drifted further, not less. This is a **dead-zone-past-the-cliff** design flaw, not a sign the underlying incentive direction was wrong.

**Binding for the next attempt**: do not repeat "align the fade's zero-point exactly with the hard eval gate" — it removes gradient exactly where a converged, low-exploration policy needs it most. The next candidate is a fade that stays continuous well BEYOND the observed operating range (e.g. `full_mm` small like 10-15mm for a strong preference toward tight, `zero_mm` pushed out to ~90-100mm so the currently-drifting 47-90mm band still has a real downward slope) — steeper than the legacy 40/80 band everywhere, but never flat, so the pull-back pressure that the legacy band provided is preserved while still being tighter than before. This is genuine reward-shaping design work (verify the gradient is nonzero across the checkpoint's actual visited fp_mm distribution before relaunching) — flagging **DIG-IN** for a deep-model cycle rather than guessing a second set of endpoints blind. The sibling arms (`clipsched-s1-fpreprice`, `retention-s1-fpreprice`, same cfg, different warm-start seeds) were still training at write time and were NOT verdicted by this entry; read their own probes before assuming this is seed-general (plausible given the mechanism is structural, not seed-specific, but not yet confirmed). `cw-stand50hz-stance-tuckclock-scratch6m-dqfix-clipsched-fpreprice` itself carries an `in-cycle` triage claim from a concurrent cycle at write time — this entry is a same-run analysis CONTRIBUTION, not that run's verdict; read its own ledger entry for the actual PASS/FAIL call. Evidence: `/tmp/fpreprice_flatprobe_det.json` (pulled from `hexapod-mjx-train-1:/workspace/prototype_sts3215/logs/ckpt_eval/cw-stand50hz-stance-tuckclock-scratch6m-dqfix-clipsched-fpreprice_flatprobe_det/report.json`); `logs/experiments/cw-stand50hz-stance-tuckclock-scratch6m-dqfix-clipsched-fpreprice/wandb_history.csv` (`env/rise_score`, `env/reward_rise_score_prog`, `env/reward_rise_score_hold`); code `exp/stand50hz-footprint-fade-reprice` (`d04ae5b6`).

## SHARPENING (2026-09-11 ~01:4x, dig-in cycle on clipsched-s1): the stand50hz footprint defect's target variable is the XY FOOTPRINT SPLAY (`_curl_dist` 46-48mm vs the 40mm PLANT_SPEC band), NOT the "~-25mm end_clear offset" the two entries below name — end_clear is a lineage CONSTANT shared by PASSING episodes

Binding correction before the flagged reward-reprice DIG-IN designs against the wrong variable: the dqfix parent's own 11/12-PASSING flat-probe episodes carry the IDENTICAL `end_clear_mm` = -25.0 to -25.2mm on every foot (`logs/ckpt_eval/cw_stand50hz_stance_tuckclock_scratch6m_dqfix_flatprobe/report.json`), so "-25mm clearance" cannot be the defect fingerprint — it is a constant of the standing pose measurement across the whole lineage, passers included. The failing `plant_fail=['footprint']` sub-check is `footprint_ok` = `_curl_dist()` (mean body-frame XY distance from each foot to its walk-champion plant anchor, sim_env.py) <= 40mm. Measured directly this cycle (eval_checkpoint.py now stores `footprint_err_end_mm`; evidence `logs/ckpt_eval/stand50hz_fpquant_{clipsched_s1,dqfix}/report.json`): failing clipsched-s1 plants at **fp = 46-48mm** (only 6-8mm over the band; com_margin 185-187mm), passing dqfix parent at **fp = 23mm** (com_margin 162-163mm). Mechanism, from the reward code (sim_env.py `plant_f`): the footprint term `fp_f = clip((80 - fp_mm)/40, 0, 1)` pays FULL income up to 40mm and fades only linearly to zero at 80mm — at the converged 47mm the policy keeps ~83% of plant-gated income while the wider splay buys +23mm CoM support margin and lower current, so the reward optimum sits just outside the hard 40mm eval cliff (08-21 ruling: reward<->eval misalignment). Two further design-relevant facts: (1) the same off-anchor drift shows in HOLD mode across the entire lineage INCLUDING the dqfix parent (every hold episode starts at `start_kind='plant'`, i.e. ON the anchors, and ends footprint-fail at herr 1-7mm) — the wide stance is the recipe's preferred stationary equilibrium generally, not a flat-rise-specific artifact; (2) the miss is small and the skill otherwise complete (rise/hold/lower all clean but for the splay), so a tightened end-state footprint fade (full pay only <=~25-30mm, zero by 40mm, active once the height ref completes — mid-rise wide poses stay unpriced per the deliberate k_rise_posture_pen design note in sim_env.py) plausibly fixes it via warm-start fine-tune rather than a from-scratch rerun. Everything else in the two entries below stands (seed-robust, mechanism-independent, four checkpoints, do-not-fund training-dynamics levers).

## CLOSING UPDATE (2026-09-11 ~01:2x): the retention-s1 control (crash-free trajectory) also converges to the identical footprint-mismatch — the cross-lineage convergence below is now confirmed on FOUR independent checkpoints, including one that never crashed at all

`cw-stand50hz-stance-tuckclock-scratch6m-dqfix-retention-s1` (the matched no-clip-schedule control from the same batch as the entry immediately below — unmodified dqfix recipe, `--snapshot-every` for retention only) finished with a completely healthy, uncrashed 6M-step trajectory: reward rose every quarter (-37.9/91.7/462.4/570.2), hold/lower both clean at the final read (herr 0.9-2.3mm / 0.0-2.4mm, zero terminations), no dqfix-style simultaneous multi-clause collapse. Its own flat-pinned probe (on-pod train-4, DR-0, det+sto n=6/mode) STILL reads rise valid_plant 0/12, `plant_fail=['footprint']` in all 12 episodes, `end_clear_mm` pinned at -25.0 to -25.2mm — statistically the same offset as `cont2m` (-25.5mm) and both `clipsched` seeds (-25.1 to -25.7mm), this time with ZERO over-current co-occurrence. This is the fourth independent checkpoint to converge on the identical signature, and the first with a fully crash-free training run behind it, which settles the question the entry below left open: the footprint defect cannot be a late-training-instability artifact (there was no instability event in this trajectory), so it is a converged reward/geometry-pricing property of the dqfix rise task under a flat start, full stop. The tail-clip-range schedule and plain-continuation retraining are BOTH correctly refuted as fixes for this clause (both stay valid, useful tools for a genuine training-instability problem elsewhere). Binding conclusion unchanged from the entry below, now on stronger evidence: next escalation is a repriced footprint/current reward term for the rise task, not a further training-dynamics lever. DIG-IN stays flagged for that design work. Evidence: `ops.sh entry cw-stand50hz-stance-tuckclock-scratch6m-dqfix-retention-s1`; `logs/ckpt_eval/cw_stand50hz_stance_tuckclock_scratch6m_dqfix_retention_s1_flatprobe/report.json`; W&B `1qzce98k`.

## The stand50hz rise-clause footprint-mismatch is a SEED-ROBUST, MECHANISM-INDEPENDENT stance-basin defect, not a training-instability artifact — two different late-training fixes (plain continuation, tail clip-range schedule) both leave the IDENTICAL ~-25mm foot-clearance offset intact (2026-09-11 ~01:1x, standwalk track)

One plain sentence: whatever late-training instability fix is used to recover hold/lower on the dqfix stance lineage, the rise clause's flat-start footprint always converges to the same wrong foot clearance, so the real defect is a converged reward/geometry basin, not the PPO tail-instability everyone was chasing.

`cw-stand50hz-stance-tuckclock-scratch6m-dqfix-clipsched{,-s1}` (the tail clip-range-anneal mechanism, `--clip-range-final=0.05` tightening 0.2->0.05 over the final 50% of steps, built+launched by the prior cycle for the dqfix late-tail approx_kl spike) both finished and were probed on-pod (flat-pinned, DR-0, det+sto n=6/mode). The mechanism DID fix its own target: hold+lower are clean both seeds (6/6+6/6 zero-termination, herr 0.5-8.2mm, no dqfix-style simultaneous multi-clause crash — `SCORE/hold_total_reward` 637/576 at final vs the crashed dqfix's 98.5). But rise FAILS decisively both seeds: valid_plant 0/12 each (need >=10/12), `plant_fail=['footprint']` in 24/24 pooled rise episodes, foot `end_clear_mm` pinned at a near-zero-variance **-25.0 to -25.7mm across every foot, every episode, both seeds** — numerically the SAME offset `cont2m`'s plain +2M continuation found independently (~-25.5mm, entry below). Seed-0 additionally tripped `over_current` on 3/12 episodes (seed-1: 0/12, cleaner) — a co-occurring symptom, not the primary defect. `train/approx_kl` still spikes 0.12-0.45 in the final 500k of clipsched-seed0 despite `clip_range` shrinking to 0.055-0.078 (`clip_fraction` stays ~0.45-0.48 the whole tail) — the schedule bounded the *damage* (hold/lower survived) without eliminating the KL volatility itself, and either way the footprint defect tracks neither lever: it is identical whether the checkpoint recovers via plain continuation (cont2m, no clip schedule) or via this tail-clip mechanism (clipsched, both seeds) — a training-dynamics fix cannot be the cure for a defect that survives two different training-dynamics fixes unchanged. Video (`rise_det_0`) confirms this is a quiet quantitative miss, not a gross failure: clean upright level tripod rise, no fall/drag/tip.

**Binding for the next stand50hz rise attempt**: do not fund a third training-dynamics lever (tail LR decay, `log-std-anneal-frac=1.0`) on this clause — two independent mechanisms already refute that class. The needed fix is a repriced footprint/current reward term (or an explicit foot-clearance/XY-band shaping term) that directly targets the ~25mm converged offset, i.e. real reward/env-code design work. Flagging DIG-IN for a deep-model cycle to design that mechanism (root-cause why the reward's rise optimum sits at -25mm clearance instead of inside the walkable band) rather than guessing another parameter. The `dqfix`/`cont2m` root-cause chain (late-training PPO instability -> hold/lower crash) stays correct and closed; this is a SEPARATE, pre-existing defect the instability fixes never touched. Retention-s1 (same-cycle sibling arm, snapshot-selection-only control, no clip schedule) is owned by a concurrent cycle — read its own verdict for whether an earlier snapshot within a single run ever clears the footprint band (informative but not expected to overturn this cross-lineage/cross-seed convergence).

Evidence: `ops.sh entry cw-stand50hz-stance-tuckclock-scratch6m-dqfix-clipsched{,-s1}` (verdicts); `logs/ckpt_eval/cw_stand50hz_stance_tuckclock_scratch6m_dqfix_clipsched{,_s1}_flatprobe/report.json`; `logs/experiments/cw-stand50hz-stance-tuckclock-scratch6m-dqfix-clipsched/wandb_history.csv` (`train/approx_kl`, `train/clip_fraction`, `clip_range_anneal/value`); W&B `aox1ypqm`/`nbfspeos`.
