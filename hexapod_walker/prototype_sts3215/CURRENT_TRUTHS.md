...
## INTERIM, NOT YET A VERDICT: the new footprint-rent mechanism's first data point (k=5, det) makes rise WORSE, not better — a NEW co-failure (height undershoot) never seen in this saga before — DIG-IN flagged, do not self-adjudicate the grid yet (2026-09-11 ~04:2x, standwalk track, triage cycle)
...
**This is NOT yet a verdict** — a fork-deciding, anomalous-vs-established-baseline result per the model-tiering trigger rules: (1) k5's own sto read is still pending (probe launched, registered via `evalpending`, not sleep-polled), (2) the k15/k45 dose siblings (also launched this cycle, same base/protocol) and a concurrent cycle's `k0ctl` attribution control (k=0, same explore-restore protocol, reward otherwise bit-exact — needed to separate "the rent term broke height tracking" from "the explore-restore fine-tune protocol itself is what's unstable, independent of the rent term, and the fpwiden-explore family's clean hold/lower simply never exercised this path") are all still resolving at write time. **Binding for the next read**: do not accept or refute the rent mechanism from this one data point; read all 4 arms (k0ctl, k5, k15, k45) together once complete. If k0ctl (rent OFF) shows the SAME height undershoot, the explore-restore protocol itself (not the rent term) is destabilizing rise-height tracking on this lineage under 2M steps — a different, protocol-level problem the k-dose comparison can't diagnose, and the fix path is more budget or a gentler log-std schedule, not a rent redesign. If k0ctl stays height-clean (matching the established 66-107mm footprint-only band) while k5/k15/k45 all show height failures, the rent term itself is destabilizing an unrelated part of the reward landscape (plausibly: a policy trying to avoid the rent's growing debit takes actions that also miss the height ramp) and needs the ramp-weighted form change already anticipated in this arm's own pre-registered gate text, not a smaller dose. **DIG-IN flagged** for a deep-model cycle to read the completed 4-arm grid (k0ctl/k5/k15/k45, det+sto, all with height/footprint/current breakdowns) and adjudicate. Evidence: `/tmp/k5_det_report.json` (pulled from `hexapod-mjx-train-4:/workspace/prototype_sts3215/logs/ckpt_eval/cw_stand50hz_stance_tuckclock_scratch6m_dqfix_retention_s1_fprent_k5_flatprobe_det/report.json`); sibling arms' probes registered in `.state/pending_evals.json` (`cw_stand50hz_dqfix_retention_s1_fprent_{k5,k15,k45}_flatprobe_{det,sto}`); `rl_docs/runs/cw-stand50hz-stance-tuckclock-scratch6m-dqfix-retention-s1-fprent-k0ctl.md` (concurrent cycle's control, hypothesis/gate text).
