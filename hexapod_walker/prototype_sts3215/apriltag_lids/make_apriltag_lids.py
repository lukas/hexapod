...
def tag_grid(tag_id: int) -> list[list[int]]:
    ...
    if 0 <= tag_id < len(TAG36H11_CODES):
        ...
        # Use the same verified dictionary as the snap-on flags for new IDs.
        dictionary = cv2.aruco.getPredefinedDictionary(cv2.aruco.DICT_APRILTAG_36h11)
        if not 0 <= tag_id < len(dictionary.bytesList):
            raise ValueError(f"tag ID {tag_id} outside tag36h11 0..{len(dictionary.bytesList)-1}")
        marker = cv2.aruco.generateImageMarker(dictionary, tag_id, 8, borderBits=1)
        image = np.ones((TOTAL_WIDTH, TOTAL_WIDTH), dtype=np.uint8)
        image[1:9, 1:9] = np.rot90((marker > 127).astype(np.uint8), 2)
        return image.tolist()
...
def write_3mf(path: Path, tag_meshes: list[tuple[int, trimesh.Trimesh,
                                                  trimesh.Trimesh]],
              *, arrange: bool, columns: int = 3) -> None:
...
def _patch_bambu_project(path: Path, thumbnail: bytes | None = None) -> None:
    ...
    if settings_key in entries:
        ...
        if "H2D" in BAMBU_MACHINE_PROFILE.name:
            # CLI exports omit installed-nozzle counts; explicitly use one
            # standard 0.4 mm nozzle per extruder for a sliceable H2D project.
            settings["extruder_nozzle_stats"] = ["Standard#1", "Standard#1"]
            settings["filament_map"] = ["1", "2"]
        settings["wipe_tower_x"] = ["35" if "H2D" in BAMBU_MACHINE_PROFILE.name else "15"]
...
def main() -> None:
    ...
    parser.add_argument("--ids", default="1-16",
                        help="comma/range list within tag36h11 0..586 (default: 1-16)")
    parser.add_argument("--output", type=Path, default=HERE / "out")
    parser.add_argument("--printer", choices=("a1", "h2d"), default="a1")
    args = parser.parse_args()
    if args.printer == "h2d":
        global BAMBU_MACHINE_PROFILE, BAMBU_PROCESS_PROFILE, BAMBU_PLA_PROFILE
        root = BAMBU_MACHINE_PROFILE.parent.parent
        BAMBU_MACHINE_PROFILE = root / "machine/Bambu Lab H2D 0.4 nozzle.json"
        BAMBU_PROCESS_PROFILE = root / "process/0.20mm Standard @BBL H2D.json"
        BAMBU_PLA_PROFILE = root / "filament/Generic PLA @BBL H2D.json"
    ...
    write_3mf(set_path, generated, arrange=True, columns=4 if args.printer == "h2d" else 3)
    bambu_path = output / (
