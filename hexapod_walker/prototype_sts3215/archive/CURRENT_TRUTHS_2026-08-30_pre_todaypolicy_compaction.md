...
Six registered tracks until all gates are green (operator 08-23/08-30
updates): (1)
...
ranking before every mechanism launch; (5) `standwalk` — mesh/100 Hz
stance plus walk distillation toward ONE policy that can sit -> rise
-> joystick-walk -> lower by itself; (6) `todaypolicy` — delivery
track for a useful policy-controlled MuJoCo/controller bundle TODAY,
explicitly allowed to compose policy+state pieces while `standwalk`
continues the single-policy goal in parallel.
