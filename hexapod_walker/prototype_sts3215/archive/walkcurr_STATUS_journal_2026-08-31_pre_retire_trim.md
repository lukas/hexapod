...
finding"): **this track is RETIRED.** Plain English: 15+ independently
designed non-BC mechanism/architecture/reset-diversity/action-space
classes across the whole campaign (14 pre-08-30 + this final
literature-informed action-box wave, 2 seeds) all converge on the
same static-stand local optimum under a from-scratch/prior-free PPO
diet on this sim/reward stack — the scope finding is that prior-free
discovery alone does not escape the initial-standing basin at this
budget scale, not that walking itself is unreachable (the `joystick`/
`standwalk` tracks' BC-anchored/teacher-distilled lineages already
walk). No further walkcurr rung-1/litrep-style arms will be launched
by the agent fleet. `STATUS.md` and `rl_move/orchestrator/tracks.json`
updated same cycle. Evidence:
`logs/ckpt_eval/cw_walkcurr_litrep_box_s0_gate/`,
