...
- **The L0 knee horn slips.** Before the re-zero the L0 knee encoder read +32 deg with the leg visibly straight; after re-zeroing, every scripted walk left it 30-32 deg from walk-ready (L0 hip drifted to +39 as well) and the gait refused the next command with "not at walk-ready pose (max dq=32 deg on j2)". The standing hold showed no sag, so this is slip under the gait's load, not a soft servo. Tighten the L0 knee horn before comparing gaits; the two-minute hold sag is otherwise gone (0.8 deg or less on every knee, from 2-6 deg yesterday).
