...
static void cmdHello() {
  // baud= lets the host log which rate won; ring= is the idle TX ring free
  // space, i.e. the Arduino core's serial ring size (TX and RX rings share
  // CONFIG_ARDUINO_API_SERIAL_BUFFER_SIZE), which bounds how much of a
  // 113-byte host frame can queue while a servo-bus pass is not pumping.
