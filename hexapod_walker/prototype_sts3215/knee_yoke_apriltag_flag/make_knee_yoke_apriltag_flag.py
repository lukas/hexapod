...
CUP_SLOT_W = 0.9
CUP_SLOT_START = 1.0
DEFAULT_THROAT_CLEARANCE = -0.10
# Solid cup pedestals move the plate outward without changing head engagement.
PLATE_STANDOFF = 2.0
INNER_BACK_CLEARANCE = 0.25

BACK_LABEL_PIXEL = 0.80
BACK_LABEL_GAP = 0.16
BACK_LABEL_DIGIT_GAP = 0.56
...
def build_holder(
    *,
    ...
    cup_height = head_height + HEAD_TOP_CLEARANCE + PLATE_STANDOFF
    ...
    for x, y in bolt_centres_about_output():
        ...
        cavity = frustum(
            back_radius,
            throat_radius,
            PLATE_T + PLATE_STANDOFF - 0.03,
            total_height + 0.20,
        )
        cavity.apply_translation((x, y, 0.0))
        cavities.append(cavity)

        slot_height = cup_height - PLATE_STANDOFF - CUP_SLOT_START + 0.25
        slot = trimesh.creation.box(
            extents=(CUP_OUTER_D + 1.0, CUP_SLOT_W, slot_height)
        )
        slot.apply_translation(
            (x, y, PLATE_T + PLATE_STANDOFF + CUP_SLOT_START + slot_height / 2.0)
        )
        slots.append(slot)
    ...
    dimensions = {
        ...
        "plate_outward_offset_mm": PLATE_STANDOFF,
        "head_engagement_mm": head_height,
        ...
        "back_label_digit_height_mm": 5 * BACK_LABEL_PIXEL + 4 * BACK_LABEL_GAP,
    }
