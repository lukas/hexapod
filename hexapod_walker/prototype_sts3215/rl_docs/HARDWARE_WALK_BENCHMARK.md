...
Queueing leaves it `waiting_for_operator`; that legacy status means waiting for
the serialized guarded runner, not waiting for a person. This program has no
network or motion code. The guarded supervisor verifies every listed
prerequisite before starting one job; a fresh camera plus three advancing,
healthy 18/18 telemetry samples is sufficient when it conclusively establishes
normal state. Hands-on help is needed only for a persistent or inconclusive
...
These numbers illustrate the file format, not a hardware result. Calibrated
output requires an explicitly selected, content-addressed Robot Lab calibration
archive whose request, report, pose configuration, and historical layout hashes
match the supplied replay snapshots. Robot Lab intentionally leaves archival
records `current:false` and `replay_ready:false`; explicit offline selection is
not activation. Motion is projected onto the initial body heading plus the
constant commanded direction; course error is the angle of net displacement,
and yaw change is reported separately. A
