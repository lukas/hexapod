...
| `tilt_settle_grace_s` / `tilt_settle_ramp_s` | 0.0 / 0.0 | ANTI-TILT SETTLE WINDOW (08-29, `cw-walkcurr-sac-sv-tilt10-s1-r2` FAIL: raising `k_roll`/`k_pitch` past dose 5.0 REVERSED a real monotone dose-response instead of continuing it — deterministic instant `tilt_pitch` collapse, `gait_valid` regressed 5/6→0/6, `roll_peak` actually LOWER than the lower doses — consistent with the charge over-taxing the vulnerable post-spawn recovery window, not with better balance). `env.py compute_reward`'s `tilt_settle_scale` multiplies ONLY `r_roll`/`r_pitch` (never `k_track`'s kernel or anything else): 0.0 for the first `tilt_settle_grace_s` seconds after `reset()` (episode-absolute clock, `sim_env.py`'s `self._step_i`; NOT reset by mid-episode `mode_seq` switches — the window is anchored to spawn), then linearly ramping 0→1 over the following `tilt_settle_ramp_s` (0 ramp = a hard step at the grace boundary). Default 0.0/0.0 = `tilt_settle_scale` always 1.0 = bit-exact with every pre-08-29 caller; the hardware env never passes it. Bank: `WALKCURR_SV_TILT_SETTLE` (`test_task_semantics.py`, 4/4 green — default-off bit-exact, ranking intact (travel > every stationary form > wrong-way > topple-as-strict-floor) at the full 10.0 dose with a 0.5s/0.5s window). |
...
| `k_walk_overspeed` / `walk_overspeed_tol` | 0 / 0.10 | **commanded-band exceedance charge** (08-20, fast-profile prep fb_20260820T000059 item 3b — the steer5-fastprof1 canary ran 2.5x the command with the legacy stack: the Gaussian kernel saturates ~2σ out and `k_walk_prog`'s 1.25 cap still PAYS overspeed): −k·min(over/s_ref, 3) per commanded-motion tick, over = max(0, planar speed − (1+tol)·s_ref). Linear (gradient never saturates), zero inside the band by construction, capped 3× for bounded per-tick cost; at the canary's operating point (0.14 m/s vs 0.06 cmd) k=2 charges ~2.4/tick vs the ~2/tick kernel income. Never gated/shrunk (charge). Stop/settle segments never charged (stop is priced by the stop→stance-hold contract). Bank: FASTPROF (`test_task_semantics.py`), units `test_walk_fastprof_mdp.py`. |
| `k_walk_heading` / `walk_heading_min_speed_m_s` | 0 / 0.01 | **direct heading-error charge** (same directive — the canary drifted 50–60° off the commanded direction): −k·(1 − cos∠(v, v_ref)) on commanded-motion ticks actually moving (≥ the min-speed floor; heading is undefined near zero speed — parking is priced by the prog gates, not here). Smooth, bounded [0, 2k]; 55° costs ~0.43k/tick, perfect heading 0. Never gated/shrunk. Same banks as above. |
| `k_walk_course` / `walk_course_tau_s` / `walk_course_min_speed_m_s` | 0 / 0.75 / 0.01 | **stride-averaged course charge** (08-22, operator reward-alignment order fb_20260822T032514 — phasedir1's PPO reward improved while dir_err went 35.6→67.3° and rear headings collapsed; nothing active priced course, and per-tick `k_walk_heading` would charge the honest tripod's ~35° stride sway): EMAs the body-frame planar velocity over tau (default 0.75 s = one teacher gait period so sway averages out per cycle), then −k·(1 − cos∠(v_ema, v_ref)) per commanded-motion tick once the smoothed speed clears the floor. Near-free for the obedient gait (measured −3.5/250 ticks at k=2), ~2k/tick for command-ignoring travel on a rear command. Never gated/shrunk; parking/refusal priced by `k_walk_idle_charge` + prog gates, not here. Bank: PHASEDIR (`test_phasedir_semantics.py`). |
| `k_walk_course_overspeed` / `walk_course_overspeed_tol` | 0 / 0.05 | **EMA-speed band exceedance charge** (same order): −k·min(over/s_ref, 3), over = max(0, |v_ema| − (1+tol)·s_ref), on the same course EMA. Exists because the INSTANT `k_walk_overspeed` cannot separate the measured 1.3–1.7× attractor (per-tick exceedance too small while `k_walk_prog` pays 1.25×) and a tight instant band taxes the honest gait's per-stride speed pulses. Scripted teachers saturate below the band at the 0.08 command, so this is insurance against RL-invented overspeed mechanics (phasedir1 measured 0.139 m/s); priced by construction, same form as the proven instant charge. Bank: PHASEDIR. |
| `k_walk_course_disp` / `walk_course_disp_window_s` / `walk_course_disp_min_speed_m_s` (+ `k_walk_course_disp_overspeed` twin with the same tol/along/ref-floor knobs) | 0 / 1.5 / 0.02 | **net-displacement course charge** (08-29 standwalk DIG-IN, k_walk_course fix-lever (b)): same −k·(1 − cos) form as `k_walk_course` but on the TRUE net body-position displacement over a trailing window instead of a velocity EMA — immune to intra-stride sway AND the slow-zigzag cancellation that left the EMA term 0/5899-tick INERT on the failed unified lineage while its real path was 6° off course. The overspeed twin is an independent key so removing the EMA term never silently drops overspeed protection. Bank: WALK_COURSE_DISP (`test_course_disp_semantics.py`). CAUTION (08-29 income-bank measurement): at k=2 this buries sustained sideways/backward travel 300–1800 BELOW park, inverting the operator's wrong-way > park ordering — pair a small dose (~0.15) with `k_walk_course_income` + refusal pricing rather than a large dose alone. |
| `k_walk_course_income` / `walk_course_income_window_s` / `walk_course_income_deadband_deg` / `walk_course_income_sigma_deg` / `walk_course_income_min_speed_m_s` / `walk_course_income_over_tol` / `walk_course_income_over_ramp` / `walk_course_income_support_gate` | 0 / 0.75 / 6 / 20 / 0.01 / 0.05 / 0.5 / 1 | **windowed command-following INCOME — the primary moving-command objective** (operator reward-design directive fb_20260829T142239_63c818, 08-29): per commanded tick, k · support_gate · angle_factor · speed_factor on the NET body displacement over a trailing gait-scale window vs the command INTEGRATED over the same window ("did the robot advance the requested distance in the requested direction while supported?"). angle_factor = 1 inside the deadband (6° ≈ mesh/100 Hz teacher windowed p95 + margin — the teacher's honest sway is NEVER charged), Gaussian(σ=20°) beyond; speed_factor rises to 1 at full command completion and FALLS beyond (1+over_tol) — the optimum sits AT the command, never above. support_gate = product of the run's own configured anchor/loadslip/height/gait income-gate factors (a shuffle/skate/flag-leg gait earns course income at its kernel-income discount, by construction). Windows containing a stop tick pay nothing; the integrated command is the time-averaged reference across switches. A raw \|disp−cmd\| vector kernel was REFUTED before launch: the teacher's own ~0.39 completion makes the vector error speed-deficit-dominated, paying a PARK 0.33 of max. Info: `walk_course_income_{err_deg,angle_f,speed_f,support}`, `reward_walk_course_income`. Bank: WALK_COURSE_INCOME (`test_course_income_semantics.py`). |
| `k_walk_excess_sway` / `walk_sway_window_s` / `walk_sway_allow_mm` / `walk_sway_course_cap_deg` | 0 / 0.75 / 5 / 60 | **teacher-enveloped excess-sway charge** (same directive, item 4): RMS perpendicular deviation of the body path around the commanded-course line over the window, MINUS the allowance (5 mm ≈ 3× the mesh/100 Hz teacher's p95 of 1.7 mm — clean tripod sway rides free); only the positive excess is charged, −k·min(excess/allow, 3) per tick. Charged only around a roughly-FOLLOWED course (window course err ≤ cap AND net motion above floor): sustained wrong-course travel is priced by the income/course terms — double-charging it here measurably inverted wrong-way > park. Measured: unified-class ±40° zigzag pays −288/8 s at k=2 while the teacher pays exactly 0. Info: `walk_sway_rms_mm`, `reward_walk_excess_sway`. Bank: WALK_COURSE_INCOME. |
...
| `k_loadslip_excess` | 0 | direct anti-skate charge (fast anti-skate V5, operator order fb_20260820T075230_4a90c6: both steer6/steer7 fast-gait forks kept skating because the loadslip GATE only zeroes income — a skater collecting non-velocity reward is complained about, never charged): −k · max(`walk_loadslip_ratio` − `loadslip_ok`, 0) · dt per commanded tick (dt-scaled like every other per-second charge, e.g. `c_time`; realigned 2026-08-20 to match the authored desktop commit 2cb2a7b7 — see q_20260820T0830Z), the same episode-accumulated ratio the gate/harness score. Additive penalty, never shrunk by income gates; composes with `walk_loadslip_gate`. Info: `reward_loadslip_excess`. Semantics: `test_walk_fastprof_mdp.test_loadslip_excess_*`. |
...
| `k_walk_freeprog` / `walk_freeprog_cap_m_s` | 0 / 0.06 | **direction-first income with NO speed target** (operator order 2026-08-21, from-scratch anti-slip walking): REPLACES the Gaussian speed kernel and the linear progress term with `walk_freeprog_score` = clip(along/cap, −1, 1) − min(cross/cap, 1) — saturating along-command credit, cross-track/backward charge, no `|along − s_ref|` band, so the command sets DIRECTION only and going faster is neither required nor punished. Positive part rides every income gate below; the negative part is added ungated at the total (a gate must never shrink a penalty). Do NOT combine with `walk_kernel_prog_gate` (it re-introduces a speed target). Info: `walk_freeprog_score`, `walk_freeprog_cross`, `reward_walk_freeprog_pen`. Semantics: SLIPWALK bank in `test_task_semantics.py`. |
...
| `k_walk_idle_charge` / `walk_idle_speed_m_s` / `walk_idle_tau_s` | 0 / 0.03 / 1.0 | **anti-park travel floor** (operator order 2026-08-21, from-scratch anti-slip walking): −k · clip((floor − EMA(along-command speed))/floor, 0, 1) · dt per commanded tick. Every prior from-scratch gait arm collapsed into freeze or march-in-place, and slip-per-metre is meaningless at ~zero travel — this charges the SHORTFALL below a travel floor (a floor, not a band: nothing to gain above it). The 1 s EMA keeps it smooth so an honest stride's per-tick oscillation through zero is not charged. Added after the income gates, so no gate shrinks it. Info: `walk_along_ema_m_s`, `walk_idle_shortfall`, `reward_walk_idle`. Semantics: SLIPWALK bank in `test_task_semantics.py`. |
...
| `k_yaw_margin` / `yaw_margin_allow_deg` | 0 / 3.0 | **hip-yaw limit-margin charge** (08-19, operator order fb_20260818T152717 lineage — the joystick direction-switch tangle): per walk tick, each leg whose hip-yaw sits within `yaw_margin_allow_deg` of either hard stop pays −k scaled linearly by depth into the band (pressed into the stop = full k). Prices the MEASURED tangle precursor (`probe_dirswitch_tangle`: post-switch yaw_sat_frac 0.013–0.093, margins to −0.65°; rot60 exonerated) after three exposure/schedule/blend levers failed to cure it. Walk-family modes only, every walk tick (stop dwells included); honest tall gait rides 10–20°+ margins and pays ~0 (bank: `test_yaw_margin_*`, tested operating point k=2). Reads qpos only — identical on C and MJX backends. |
...
| cfg key (reward.) | default | what it does |
|---|---|---|
| `getup_k_progress` | 200.0 (was 60.0 until 08-22) | one-shot ratchet: +k·Δmax(P) where P = `getup_w_zero`·untangle + `getup_w_load`·weight-on-feet + `getup_w_stand`·S. Baseline seeds at the first tick (spawn posture is never income); regressions/re-farming pay 0 by construction. RECALIBRATED 08-22 (`test_getup_honest_ordering` dig-in): at 60 the honest partial-rise (holding a real mid-ramp crouch) earned LESS than freezing at spawn (-12.16 vs -11.26, 3 seeds) — the one-shot credit for the potential gain didn't clear the extra gyro/action/current regularizer cost of actually executing the rise (those regularizers are deliberately untouched — they price real physics). 200 restores partial > freeze with a >=20-point margin while preserving every other GETUP-bank ordering (swept 60->500). Full belly→stand ≈ +145 at 200 (was ≈+43 at 60). |
