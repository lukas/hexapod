# cw-assistfade-rung2-anchorfade-s1
...
**verdict**: CANARY PASS (mechanism-health, exceeds bar) -- same read as -s0's twin seed. Held-out gate eval 24/24 episodes gait_valid=true, 0 terminations, 0 sacrificed legs, six legs cycling (duty pattern matches s0), progress_ratio 0.35-0.51 across all 4 modes (bar >=0.35), slip/m 2.5-3.3. bc_anchor_anneal/gate_pass never latched in 2M (early_term_rate 0.25 in the n=8 training-time assay vs 0/24 in the held-out eval), anchor stayed at coef=3.0 throughout -- matches the CANARY gate's own 'not yet annealed is fine' criterion, and matches -s0's read closely enough that this is a clean seed-replicated PASS, not a lucky single seed. Evidence: logs/ckpt_eval/cw_assistfade_rung2_anchorfade_s1_gate/report.json, wandb 70k66xu7, logs/experiments/cw-assistfade-rung2-anchorfade-s1/wandb_history.csv. Next: -cont8m alongside -s0's, same rationale (more assay checks to clear ignition_gate_pass and drive the anneal, then re-check post-anneal).
