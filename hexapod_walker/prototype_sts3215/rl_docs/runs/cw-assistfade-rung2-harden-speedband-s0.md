...
**verdict**: Killed ~2min after launch: respec cloned the source's train.bc_anchor_coef=3.0/anneal_gate=1 unchanged (forgot to zero it in the respec --cfg overrides), contradicting this arm's own registered hypothesis (BC anchor off, task-only PPO on widened band) and confounding the speed-band question with a re-run of the anchor anneal. Relaunched immediately as the same run name is not reused -- see cw-assistfade-rung2-harden-speedband-s0 (v2, this cycle) with anchor explicitly off.
