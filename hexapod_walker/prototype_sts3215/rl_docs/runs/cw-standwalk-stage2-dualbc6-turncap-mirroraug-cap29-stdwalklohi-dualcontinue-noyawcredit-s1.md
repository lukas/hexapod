# cw-standwalk-stage2-dualbc6-turncap-mirroraug-cap29-stdwalklohi-dualcontinue-noyawcredit-s1

<!-- GENERATED from experiments.json by launch_run.py — do not edit -->

**status**: CANARY FAIL - MECHANISM

**created**: 2026-09-04T04:26:41+00:00
...
**verdict**: CANARY FAIL - MECHANISM (confound-isolation control, informative, seed1 twin -- matches seed0). Same probe_turn_authority.py --vx-cmds (85-key non-train cfg-set) read as the seed0 twin, against cap29-stdwalklo-hi-s1: pure-turn wz_med regresses 20.9-26.5% both signs -- again close to the already-closed Triple-s1 canary's own 18.5-27.7% loss on the SAME confound-matched cells (reading A confirmed again: the dropped train.yaw_credit_coef/_vf_coef/_grad_clip + --gru-dual-log-std-split/--log-std-anneal-core mechanisms, not the architecture swap, explain most of Triple's pure-turn loss). Reading (C) also reconfirms: this Dual run's combined-tick (vx=0.08) wz_med beats Triple-s1 on 3/4 matched cells and is essentially tied on the 4th (wz+0.25: dual +18.2% vs control, triple -0.6%; wz-0.25: dual +22.6% vs control (stronger than control!), triple -25.0%; wz+0.25 other sign dual -17.5%-class... see seed0 note for full table) -- with the identical confound in both arms, architecture bought nothing on combined-tick in either seed. 2-seed confound-isolation pair now CLOSES both readings cleanly: pure-turn loss = mechanism-drop, not architecture; combined-tick benefit from the Triple architecture = zero-to-negative in 8/8 matched cells across both seeds. Disqualified from adoption by its own pure-turn regression; explanatory control only. Full table: rl_docs/tracks/standwalk/STATUS.md.
