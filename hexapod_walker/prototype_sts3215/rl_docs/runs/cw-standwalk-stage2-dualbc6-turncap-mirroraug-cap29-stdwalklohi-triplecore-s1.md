...
**verdict**: CANARY FAIL - INFRASTRUCTURE: identical net_arch-mismatch crash as the seed0 twin (same recipe, same parent checkpoint, same fresh-Triple net_arch derivation gap) -- see that run's verdict for the full trace. Zero training steps, zero corruption. Fixed in the same commit (ff3cd275: derive net_arch/lstm_hidden_size from the loaded Dual checkpoint instead of CLI defaults). Retry s1-r2 already launched and RUNNING past the crash point on train-8.
