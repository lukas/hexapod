...
**verdict**: CANARY PASS: speed-range widening (0.03-0.12 m/s dynamic-cap command band) composes cleanly on the crutch-off full-DR composite. Evidence: 24/24 held-out episodes, gait_valid 22/24, 1 termination (walk_startjitter/sto ep2, tilt_pitch) -- same hardest cell where the parent's own s1/s2-acq1 baseline already shows scattered leg0/leg5 flags (21/24 gv, 0 terms there); sac legs confined to walk_startjitter/sto only, matching the pre-existing parent signature, no new mode affected. Why: matches the arm's own 'near-0 falls, no new chronic sacrifice' pass prediction; the single fall is an isolated hardest-cell event, not a reproducible pattern (unlike the irr siblings this cycle). Next: license the ACQ (40M) continuation to test whether speed-widening holds at acquisition depth, same pattern as the widen8 heading axis.
