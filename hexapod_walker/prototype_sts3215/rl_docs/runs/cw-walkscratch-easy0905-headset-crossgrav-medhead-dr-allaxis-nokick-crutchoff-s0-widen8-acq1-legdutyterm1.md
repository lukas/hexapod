# cw-walkscratch-easy0905-headset-crossgrav-medhead-dr-allaxis-nokick-crutchoff-s0-widen8-acq1-legdutyterm1

<!-- GENERATED from experiments.json by launch_run.py — do not edit -->

**status**: CANARY FAIL - MECHANISM

**created**: 2026-09-07T21:09:10+00:00
...
**verdict**: CANARY FAIL - MECHANISM: the new per-leg minimum-duty TERMINATION does not repair this seed's chronic leg sacrifice within a 2M-step canary -- it fires constantly instead. Evidence: walk/det gait_valid WORSENED 4/6 (pre-mechanism widen8-acq1 baseline) -> 0/6 (this run), with walk_leg_duty_terminate firing in 6/6 det episodes (vs 0 terminations baseline) and a sacrificed leg flagged in every det episode (varying: [2,5]/[0]/[0]/[0]/[0]/[5], vs 2/6 baseline). Video (walk_det_0.png/walk_det_1.png) shows the robot barely translating with legs jittering before an early cutoff, not a repaired six-leg gait. Reward is still rising (quarters 57.8/110.4/169.0/178.8, well below the 40M parent's converged level) so this is a genuine 2M-budget-too-short read, not proof the termination itself is unsound -- it exactly matches this run's own pre-registered FAIL branch ('terminations still frequent at the end = never converges'). Why: 2M steps is not enough runway for the policy to learn to avoid a brand-new hard termination it has never been penalized by before; the entrenched 40M sacrifice habit does not unlearn that fast. Next: read the 3 sibling canaries (s1/s2-widen8-acq1, s0-widenbis180) before generalizing -- if they show the same shape, the mechanism needs a longer acquisition budget (08-21-style continuation, reward still rising) before any verdict on whether it actually resolves the pathology, not a redesign.
