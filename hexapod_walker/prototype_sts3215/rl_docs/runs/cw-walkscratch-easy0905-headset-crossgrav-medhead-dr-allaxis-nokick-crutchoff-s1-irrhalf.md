...
**verdict**: CANARY FAIL - MECHANISM: halving the irr-timing jitter amplitude (0.5->0.25 on goal.walk_cmd_resample_jitter) does NOT clear the crutch-off full-DR composite's jitter+push interaction -- confirms amplitude is not the driver. gate criteria required 0 falls/24 + gait_valid>=18/24 + no new chronic sacrifice outside the parent's own walk_startjitter/sto leg0/leg5 flag. Result: 1 real fall (walk/sto ep4, term_reason=tilt_roll, roll_class=fell, roll_peak 30.3deg) -- the SAME episode index the full-amplitude s1-irr canary also fell at -- plus a NEW chronic sacrifice (leg5) spreading into walk/det (2/6) and walk/sto (1/6), modes the parent crutchoff-s1-acq1 baseline never touches (its only flag is walk_startjitter/sto leg0/leg5); startjitter/sto itself is actually clean here (6/6, better than parent's 3/6) -- the pathology relocated, it did not disappear. gait_valid total unchanged at 21/24 but the composition is qualitatively different from the clean parent. Matches this run's own pre-registered FAIL prediction (same tilt_roll fall or new chronic sacrifice reappears) almost exactly. Evidence: logs/ckpt_eval/cw_walkscratch_easy0905_headset_crossgrav_medhead_dr_allaxis_nokick_crutchoff_s1_irrhalf_gate/report.json vs ..._crutchoff_s1_acq1_gate/report.json, frame checks on walk_sto_4 (fall episode). W&B 1v4ou8gd.
