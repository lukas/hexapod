...
**verdict**: CANARY PASS PERFECT — command-drop realism (cmd_drop_prob≈0.022) restores clean on the flagship champion. Evidence: aggregate gait_valid 24/24 (all 4 modes 6/6), 0 falls/terminations, sac=[] every single episode, slip/m med 3.55-4.67 (in the established sibling-axis band). Video-implied clean six-leg cycling (no flags at all to check). Why: occasional dropped servo commands cost nothing at nominal dose, joining the long list of individually-benign axes (latency/deadband/torquefade/noise/push/mass/friction/...). Next: fold into a composite/ACQ read if a future wave wants this specific axis at 40M; not separately queued this cycle (see cycle refill for the picks made instead).
