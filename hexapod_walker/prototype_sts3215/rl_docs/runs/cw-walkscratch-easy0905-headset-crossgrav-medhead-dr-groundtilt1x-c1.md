...
**created**: 2026-09-06T05:53:27+00:00
...
**hypothesis**: Restore nominal (1x) GROUND-SLOPE spread (ground_tilt_deg: floor pitch/roll applied via a tilted gravity vector, simulating concrete/mat/carpet not being perfectly level) -- the easy0905 recipe has trained on a perfectly level floor every episode (dr-scale=0.0 collapses this to 0 when not explicitly overridden). Does the campaign's most durable champion (medhead-abrupt-c1-acq1-cont40m, 80M steps, 24/24 clean, 0 falls) survive its own nominal own-DR floor-slope spread (+-2deg) without retraining collapse? Isolated single-axis diagnostic, same template as the sibling latency/deadband/torque/noise/mass/friction/gain/geometry/zerobias DR-restoration arms -- this axis (terrain slope, distinct from the crossgrav campaign's own gravity-MAGNITUDE scaling) was not yet covered by any prior arm.
