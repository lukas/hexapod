...
**verdict**: CANARY FAIL - INFRASTRUCTURE: died in ~2s, 0 steps logged (wandb w6xs0zav exit_code 0/runtime 0 looked clean, no behavioral evidence either way). Root cause: the launching cycle's respec kept --activation-fn elu on top of a plain --init-from, tripping train_ppo_mjx.py's own SystemExit guard (PPO.load already restores the checkpoint's saved ELU; the guard exists because reasserting activation on a warm start is unsupported) -- the exact CURRENT_TRUTHS.md Known Tooling Gotchas bug, this time on a plain-elu family arm rather than a gSDE one (broadens the documented scope). Relaunched correctly as headset-halfgrav-c2 (blank --activation-fn, plain --init-from) on train-0. Superseded by headset-halfgrav-c2, not a lineage kill.
