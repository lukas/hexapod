...
**verdict**: Killed 30s after launch: auto-placed on hexapod-mjx-train-5 which had a CPU-heavy eval_checkpoint (base-s3 gate eval, started 10:38) already consuming most cores -- checkup showed fps=91 (floor 5000), starved not misplaced. Relaunching on an explicitly-free pod (no competing eval) instead of waiting out the contention.
