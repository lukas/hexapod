...
**verdict**: INFRA_FAIL, not a training/reward result: launcher's stall guard killed this 210s after launch (W&B global_step frozen at 4096, host-side fps~91) because hexapod-mjx-train-5 was CPU-contended with another job at launch time (auto-placement collision, per the same cycle's own retry note in walkcurr STATUS.md). Only 4096 steps logged -- no eval report, no video, nothing to score. A same-name retry on train-9 was correctly REFUSED (W&B names are append-only); the lineage was relaunched clean under the distinct name cw-walkscratch-easy0905-sde-s3-c1b on a confirmed-idle pod (train-9) and is VERIFIED RUNNING past the crash point (still training this cycle, left alone). No further action on this entry -- sde-s3-c1b carries the hypothesis forward.
