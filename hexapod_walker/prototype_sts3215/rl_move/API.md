...
| POST | `/api/rl/drive/start` | Start persistent held-key drive session (motion-free walk preflight from the current sim walk-ready pose; live camera/telemetry supervision). Guarded trials may supply optional `active_duration_s` (0.05–20 s), enforced on the robot from first learned-walk engagement; ordinary interactive drive leaves it unset. |
