...
## q_20260823T1750Z — wzmask2 n=5 grid CLOSES the yaw fork: SIX mechanism classes now refuted, DIG-IN flagged for the next lever
Plain English: this cycle verdicted the 3rd of five replicate draws (`wzmask2-s13`) for the gyro-discriminator-mask decider grid; a concurrent cycle's `s17`/`s23` reads closed it at the full n=5 shortly after (`rl_docs/tracks/amp/STATUS.md` top banner, ~17:4x). Pooled result: FAIL — masking the discriminator's rotation-sensing dims moves nothing (tip_left/right medians 0.2092/0.2371, both inside ±0.02 of parent pooled 0.2168/0.2351, only 1/5 draws clears 0.20 on either side, need ≥2/5).
- **State of the yaw fork**: SIX independent mechanism classes are now refuted end-to-end on this lineage — task-income pricing (`k_yaw_prog` 1×/2×/3×), demo-ceiling swap (`teacher_v3`, measured +30% kinematic ceiling on the SCRIPTED demo itself, 0.144→0.174 rad/s), AMP-style ablation (`noamp1`, traded slip instead), turn-state reset densification (`tipspawn` family), CPG demo-anchor swap (`cpgdemo1`), and discriminator-obs gyro masking (`wzmask1`/`wzmask2`, both widths, n=5 replicate-calibrated). No untried reward/demo/discriminator-obs lever remains named anywhere in the STATUS history.
- **What I did NOT do**: design or launch a new "stance-geometry/turn-authority" mechanism myself. That is a structural (policy action-space / gait-timing / possibly actuator-limit) hypothesis, not an incentive one — the `RUN_INTERPRETATION_RULES.md` root-cause-chain discipline and the track's own precedent (`~12:0x` pricing-grid closure, `~11:4x` seed-lottery/budget-stability closure) both flagged the equivalent fork DIG-IN rather than having a triage-tier cycle guess at a redesign. Flagging this the same way.
- **Concrete falsifying probe I'd recommend first** (cheap, eval-only, no training): the scripted teacher's OWN turn-in-place gait saturates ~0.15-0.16 rad/s at ANY commanded ω (measured `q_20260823T1240Z`, foot-sweep-per-stride geometry-capped) — but that number was measured for the hand-scripted `TripodGait`, not for what this hexapod's actuators/kinematics can physically achieve in place. `bus.write_speed=1500` counts/s resolves to ~131.8 deg/s per joint (`[motor-contract]` line in every eval log); whether a 0.30 rad/s BODY yaw rate is even kinematically reachable at that per-joint velocity ceiling (independent of any gait script or reward) is a pure forward-kinematics/actuator-limit calculation + a MuJoCo probe (drive an idealized/optimized turn-in-place trajectory open-loop, no RL, no AMP, and measure the achieved rate ceiling) — decides whether the miss is "policy hasn't learned it" (real, fixable, structural-mechanism lever) or "0.30 rad/s cmd / 0.20 rad/s err bar was never physically achievable at this plant" (the bar-amendment branch, `q_20260823T0130Z`, becomes the only live path). I did not run this myself this cycle (kinematics derivation + a clean open-loop probe harness is more than a triage-budget task) but it should be the FIRST thing the next dig-in cycle checks, before spending any more GPU-training arms on either branch.
- **What I DID do this cycle, unblocked by the fork**: (1) built `eval_amp_m5.py --walk-per-mode` (default `None` = old `--per-mode`, bit-exact-when-omitted, 2 new unit tests + existing merge bank green) implementing the `q_20260823T0700Z` amendment (walk-section slip verdicts have been medians of only ~3 translating episodes at the default per-mode=6); (2) launched a walk-only, eval-only (no GPU/training spend, controller CPU) re-read of the lineage's actual M5-candidate base (`...pushcal518`, pre-dose-grid) at `--walk-per-mode 24` to check whether the walk-slip bar miss (3.5-3.8 family band vs bar 3.5) is real or the named thin-sampling artifact — result pending, will land in `logs/ckpt_eval/diag_pushcal518_walkpermode24/`.
- status: RESOLVED same cycle, no dig-in handoff needed after all — a concurrent cycle ran almost exactly the falsifying probe recommended above (using the exported CPG controller artifact rather than a fresh kinematics derivation) BEFORE this note was even written: `logs/ckpt_eval/cpg_turnauthority_wz030/` shows 0.30 rad/s body yaw IS plant-reachable (~0.27-0.29 rad/s achieved, 0 falls) — bar-amendment branch answered NO, adopted answer recorded above on `q_20260823T0130Z`. That unblocked a concrete, well-specified next arm (the fork's own named "CPG-sourced turn-rate clips" lever, not a fresh redesign), so this cycle built+launched it directly instead of leaving it for a dig-in: `merge_motion_library.py` (11/11 tests) splices ONLY the turn_ccw/turn_cw families from `cpg_v1` into `teacher_v2` -> `teacher_v4.npz`, single-lever respec `-turnclip1` VERIFIED RUNNING (train-2, 2M). Also independently reconfirmed the slip-axis thin-sampling finding at n=28 translating episodes (`eval_amp_m5.py --walk-per-mode`, new tool, det_slip_med 3.553 — exact match to the earlier per-mode-12 read). Read `-turnclip1`'s `eval_amp_m5` next cycle against its pre-registered gate (`rl_docs/tracks/amp/STATUS.md` ~18:1x). No operator input needed.

## q_20260823T2110Z — FIRST full amp-m5-v1 PASS (phasehz11, 1.1 Hz): when does the amp track's M5 gate flip?
Plain English: the amp track just got its first checkpoint that passes every section of the pre-registered M5 suite (walk/yaw/push/fault, plain MuJoCo, own-cfg, walk confirmed at n_translating=28): `cw-amp-m4-turnfault-seq1-pushcont1-tipfrac05-pushcal518-phasehz11` (goal.walk_phase_hz 1.33->1.1, seed 7, 2M discovery). tracks.json says amp is DONE at M5; AMP_LOCOMOTION.md's M5 text (same actor weights replay in MuJoCo preserving command response / gait / push recovery / partial fault adaptation) is satisfied by this checkpoint as written.
- Assumption adopted (assume-and-go): a SINGLE-seed, 2M-discovery pass is not enough to flip a track gate that eight mechanism classes failed to move for days — the joystick DONE declaration precedent used a 4-seed pass set. So M5 is declared CANDIDATE-met now (checkpoint + SKILLS row recorded) and the gate flips only on the pre-registered 4-seed pass-rate read: seeds 7/17/23/29 of the exact recipe, >=3/4 full m5 PASS = DONE; 2/4 = continuations on best checkpoints; <=1/4 = seed luck, curve re-opened. Twins launched this cycle (all VERIFIED RUNNING).
- If the operator prefers the letter of the gate (one policy passes = DONE), the declaration can be made immediately on phasehz11's existing artifacts — nothing about the twins invalidates it.
- status: OPEN (proceeding on the replication read; next triage cycle owns the declaration)

## q_20260824T0233Z — walkcurr rung-1: BC-KICKSTART RULING NOW NEEDED — every non-BC mechanism this track could invent (14 classes) has failed aligned at 2M [operator]
Plain English: the walkcurr track's own founding rule (operator 08-23) is "no gait clock, no BC teacher, no motion prior" — a genuinely from-scratch PPO walk. After 9 independently-designed reward-mechanism classes (exploration noise/entropy, loadslip-bootstrap at 2 window lengths, height-gate loose+tight, park_duty confound-fix at 2 doses, RND at 3+ doses and 2 diets) all converged to the identical frozen/static-stand basin with an aligned reward (charges bite, reward falls, behavior never escapes — the 08-21 ruling's "genuine FAIL" bar), this cycle built + bank-proved a 10th mechanism (`safety.walk_idle_terminate_s`, a direct termination on sustained near-zero joint velocity — the "absorbing states beat prices, must come WITH a termination" lesson from stagea-slip1) and it is now training (`cw-walkcurr-pf-fwd6-idleterm1`).
- The track's own STATUS.md has repeatedly named BC-kickstart as escalation item (d), explicitly flagged as brushing the founding no-BC-teacher rule, "last resort" only once every reward-mechanism-side lever is exhausted. This note is that flag, filed per the "no operator pauses, record the assumption, keep moving" rule — NOT a request to pause. Assume-and-go answer adopted: do NOT build BC-kickstart yet. idleterm1 is a genuinely new mechanism class (a hard termination boundary, not another price) with a plausible causal story for why it could succeed where 9 pure-pricing levers failed (it removes the option to sit in the static basin for the full 15s at all, forcing far more frequent resets/exploration per unit wall-clock) — it deserves its own read before treating the reward-mechanism side as exhausted.
- If idleterm1 ALSO fails with the frozen-crouch/static-stand signature (aligned reward, healthy optimizer, per the 08-21 rule), the honest next step is a brief BC-kickstart (a short imitation-warm-start purely to escape the initial-basin problem, distinct from the AMP motion-prior style reward the track's rule is really aimed at) OR accepting that a from-scratch MLP-PPO walk is not reachable at this budget/architecture and the track's rung-1 gate needs either a bigger budget, a curriculum change (rung-0 sub-goal redesign beyond what's been tried), or an operator ruling on whether BC-kickstart is in-bounds despite the letter of rule (a). This cycle does NOT decide that ruling — it is a genuine values question (how strictly "no BC teacher" should be read against "make forward progress"), not a reward/eval alignment question the assume-and-go rule covers.
- status: OPEN (proceeding — idleterm1 training, no pause; will act on its own read next cycle without further operator input unless it also fails, at which point the BC-kickstart question above becomes live)
- **UPDATE 08-24 ~03:1x**: idle-termination FAILED in all now-tested configurations — `idleterm1`/`idleterm2` (raw a=0 lineage) and `actbias1-idleterm1` (the clean, non-collapsing action-bias-fixed park-stand, the mechanism's actual intended target: a "fake fidget" cheat — the policy learns to jitter unloaded legs just enough to dodge the qvel-eviction floor and survive longer, `rollout/ep_len_mean` climbing 16->471 steps while `walk_freeprog_score` regresses, reward falling every quarter — an ALIGNED FAIL per 08-21, not a mechanism-firing bug). 10-11 independently-designed rung-1 mechanism/architecture classes are now refuted (see STATUS.md tally). Per this note's own text, that makes BC-kickstart "the only unexplored escalation" — but re-reading the standing orchestrator prompt's assume-and-go clause against this specific case: BC-kickstart does not just brush the founding rule, it directly reverses an explicit, recent (08-23), binding operator ruling for this exact track ("no gait clock, no BC teacher, no motion prior"), which is categorically different from an open design/reward/gate-definition question the assume-and-go clause is written for (those are cases the operator has NOT already ruled on). **Decision (assume-and-go, this cycle): do not build BC-kickstart without an explicit operator answer to this question; instead exhaust the one credible non-BC lever this campaign never tried.** Found + smoke-tested `goal.walk_park_start_frac` (pure-procedural park-posture reset diversification — no reference motion, no teacher, no gait clock — already built pre-walkcurr and historically proven to fix an analogous park-persistence failure on a different lineage, `cw-walk-parkstart-mjx-c1`). Launched a 2-arm dose pair (`-parkstart-p25` 0.25, `-parkstart-p50` 0.5) off the clean actbias1 base. `-p25` has already FAILED (aligned, static park-stand persists, `ep_len_mean` rises 64->490 — the exit gradient exists but re-standing still out-prices stepping); `-p50` is the last pending read.
- **If `-p50` ALSO fails (same static-stand signature)**: reset-state diversity is exonerated alongside every reward/architecture/termination lever tried, meaning ALL non-BC options this track has been able to name are exhausted at the 2M discovery budget. At that point the honest position is genuinely binary and this note's status becomes load-bearing rather than advisory: (a) an operator ruling on BC-kickstart's in-bounds-ness (the question this note asks), or (b) the track formally records rung-1 as BLOCKED-pending-operator (not spend, not physical-robot, but the closest analogue this track has produced — every other lever this cycle could invent has been tried) while the fleet's effort concentrates on the amp/joystick/cpg maintenance items and any later-rung walkcurr work that does NOT require solving discovery-from-scratch first (there is none — every later rung is gated on a rung-1 pass). Recorded here explicitly so the cycle that reads `-p50` does not need to re-derive this reasoning.
- status: OPEN, escalating in priority — this is now the single decision blocking further walkcurr progress once `-p50` reads.
- **UPDATE 08-24 ~03:3x**: `-parkstart-p50` has now FAILED too (same signature as `-p25`: `ep_len_mean` climbs toward the episode max, `walk_freeprog_score` bumps then regresses, reward falls every quarter, static recovered stand on video, zero gait_valid). Reset-state diversity is CLOSED at both tested doses (0.25/0.5, dose-insensitive). Correction to the prior update's framing: this does NOT yet mean literally every non-BC lever is exhausted — two sibling arms from a concurrent cycle are still pending (`cw-walkcurr-pf-fwd6-rscale50-sde-actbias1`, testing whether gSDE's forward-excursion tendency survives ON TOP of the stable action-bias-fixed base without the idle-terminate confound, and `-sde-s2`, a seed replicate of the sde forward-excursion finding itself) — those ARE still a credible, already-in-flight, non-BC combination line and should read before this question is treated as fully exhausted. If BOTH of those also fail with the same static-stand-or-fall signature, then yes: every reward-pricing, termination, architecture/exploration-structure, and reset-state-diversity mechanism this track has been able to invent (12+ classes) will have failed at the 2M discovery budget, and this note's status genuinely becomes load-bearing (an operator ruling on BC-kickstart, OR the track formally records rung-1 progress as blocked pending that ruling while the fleet works amp/cpg/joystick maintenance items — there is no lower rung to fall back to since every later rung is gated on a rung-1 pass). Whichever cycle reads `sde-actbias1`/`sde-s2` should make that final call rather than re-deriving this chain.
- **UPDATE 08-24 ~03:4x — BOTH read, BOTH FAIL. This is now the final position.** `sde-s2` FAILed first (the sde-only lineage's forward-progress claim was seed-luck on noise DIRECTION, not a real mechanism — det stayed the identical static crouch in both seeds). `sde-actbias1` (the decisive combination) then FAILed WORSE than either parent alone: det gate 0/6, all six episodes collapse to the bit-identical degenerate pose (fwd 0.02m, slip/m 11.07, sac [0,1,3] every single episode); sto gate 0/6 with 5/6 real falls (`TERM tilt_pitch`, video-confirmed progressive topple) — gSDE noise destabilizes the stable action-bias base instead of producing directed walking. `env/walk_freeprog_score` sat in [-0.15,-0.20] the entire run, WORSE than the [-0.10,-0.05] band every other rung-1 arm converged to; `ep_len_mean` rose then FELL (falls got more frequent with more training, the only arm in the whole campaign to show this shape); clip_fraction was the healthiest of any rung-1 arm (0.12-0.16) — an unambiguous ALIGNED FAIL, not an optimizer artifact.
  **Tally: 14 independently-designed rung-1 mechanism/architecture/reset-diversity classes now refuted, every one an aligned FAIL (reward flat-or-falling, eval flat/bad, healthy optimizer, 2M-6M budget)**: init-noise, entropy, loadslip-bootstrap (2 windows), height-gate (2 doses), park_duty (confound+dose+actbias-stack, 3 combinations), RND (3 doses + budget + 2 diets), rung-0 swing-income (2 doses), GRU (2 doses), gSDE (2 doses + the actbias combination + a seed replicate), reward-rescale (2 doses + 2 continuations), idle-termination (3 configurations), park-start reset-diversity (2 doses). **No further non-BC mechanism is pre-registered, credible, or has been left un-tried by this campaign.** A from-scratch MLP-PPO policy reliably converges to a stable, non-collapsing, static stand (or an unstable variant that falls) under every reward shape, termination boundary, architecture, exploration structure, and reset-state distribution tried at the 2M (up to 6M with continuations) discovery budget for this recipe.
  **This question is now genuinely load-bearing, not advisory.** Per the standing orchestrator prompt's own carve-out ("the only grounds to decline [an operator order] are typo-level mistakes, genuine safety violations, unrepairable failing tests/preflight, or mechanical impossibility — never policy objections" — read together with "an explicit recent operator ruling is not an open design question the assume-and-go clause licenses overriding"), the fleet will NOT unilaterally add a BC-kickstart to the walkcurr recipe, because that would mean overriding the track's own explicit, deliberate, recently-stated founding rule (a) on my own initiative, not merely filling an unaddressed gap. Two honest paths forward, both requiring the operator: **(1)** rule that a brief BC-kickstart (imitation warm-start solely to escape the initial-state basin, not an ongoing motion-prior reward term) is in-bounds despite the letter of rule (a) — if so, the fleet will build and bank-prove it next cycle without further pause; **(2)** rule that it is NOT in-bounds, in which case walkcurr rung-1 (and therefore every later rung, all gated on a rung-1 pass) is genuinely blocked at the from-scratch-MLP-PPO architecture/budget this track has specified, and the track's STATUS.md should record that explicitly as a scope/architecture limit rather than an in-progress campaign — the fleet's effort would then concentrate entirely on amp/cpg maintenance (both gates already GREEN, [operator]-owned past this) and joystick's remaining polish items, with walkcurr correctly parked as `[operator]`-blocked (analogous to the physical-robot/spend-approval carve-outs) rather than treated as runnable work.
- status: **OPEN — [operator] decision required.** No pause on the rest of the fleet (amp/cpg/joystick continue on their own merits); no further walkcurr rung-1 arms will be launched by the agent fleet until this is answered, since every mechanism it could invent inside rule (a) has been tried and failed.
- **CORRECTION 08-24 ~03:5x**: the "no further mechanism inside rule (a)" claim above was written before checking this file's own STATUS.md, which had already pre-registered one more untried, rule-(a)-compliant axis: an episode-length/reset-frequency curriculum (short TRAINING episodes to buy more resets per step budget, distinct from reward pricing, architecture, or reset-POSE distribution — no gait clock, no BC teacher, no motion prior, so it does not touch the operator ruling this question is about at all). That entry explicitly said "worth building only if sde-actbias1 also closes the fork — do not build it speculatively before that read." This cycle independently verdicted `sde-actbias1` FAIL (confirming the closing-fork condition) and, per the pre-registration, built + unit-tested + smoke-verified `--training-episode-seconds` (`train_ppo_mjx.py`, bit-exact default) and launched a 2-arm dose pair (`cw-walkcurr-pf-fwd6-actbias1-shortep3` / `-shortep8`, VERIFIED RUNNING train-2/train-4) rather than treating the question as immediately final. This is NOT a walk-back of the operator-ruling question itself (BC-kickstart is still untouched, still awaiting the operator) — it is exhausting the truly last rule-(a)-legal lever before the "OPEN, no further arms" framing above is accurate. If `shortep3`/`shortep8` BOTH fail with the same static-stand-or-fall aligned-FAIL signature, THEN the framing above is exactly correct with no correction needed, and the next cycle to read that double-FAIL should treat this question as fully load-bearing without re-deriving any further lever.
...
- **ANSWERED by the operator, 2026-08-29 (fb_20260829T145710_06f739, Lukas via local Cursor agent): BC-kickstart is OUT OF BOUNDS.** Any imitation warm-start, however brief, including "solely to escape the initial-state basin", is not allowed for this category of research — founding rule (a) stands as written and covers initialization. Per the question's own path (2): the 15-class finding is recorded in walkcurr STATUS.md as a finding about the from-scratch CENTRALIZED-MLP-PPO recipe at the 2M discovery budget — NOT as a permanent track scope limit. The same ruling registers operator-supplied literature (Schilling et al. IROS 2020 decentralized per-leg PPO; Azayev & Zimmermann 2020; Heess et al. 2017) proving from-scratch hexapod PPO walking is achievable, and names the untried rule-(a)-legal levers in priority order: (i) decentralized per-leg actor modules, (ii) plain-velocity discovery reward with no charge stack (semantics bank keeps gating rung PASSES eval-side), (iii) bigger-than-2M one-shot budget probe (20M+, a few seeds), (iv) off-policy SAC if PPO stays stuck. walkcurr resumes rung-1 with those arms; the eval-side WALKCURR_PF gate definition is unchanged.
- status: **CLOSED (operator-answered 2026-08-29). Ruling executed the same cycle:** rulebook/STATUS updated, `--decleg` per-leg policy + WALKCURR_SV simple-velocity bank built and proven, decleg-sv 3-seed grid + centralized-sv control launched at 20M (see walkcurr STATUS.md 08-29).
- status: **OPEN — [operator] decision required, fully load-bearing, nothing else in flight** *(superseded by the CLOSED status above; kept for the record).* Both named paths from the ~03:4x update stand unchanged: (1) rule BC-kickstart in-bounds (fleet builds+bank-proves it next cycle, no further pause), or (2) rule it out-of-bounds (walkcurr rung-1, and every later rung gated on it, is recorded in STATUS.md as blocked at the from-scratch-MLP-PPO architecture/budget this track specified — a scope/architecture limit, not an in-progress campaign). Fleet effort concentrates on amp/cpg maintenance ([operator]-owned past their gates) and joystick's V6 hardening lineage (itself currently blocked on its own small DIG-IN: a direct actuator-current charge, see joystick/STATUS.md 08-24 ~04:1x) in the meantime.
...
## q_20260908T0410Z — future contact calibration (review corrected 03:31 UTC)
...
The future fleet decision is whether calibrated contact evidence warrants a
plant-default or randomization change. No pressure distribution, effective
