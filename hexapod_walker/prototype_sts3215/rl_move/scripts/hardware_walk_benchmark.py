"""Build guarded Robot Lab specifications and audit saved walking evidence.
...
def build_plan(*, include_planted: bool = False) -> dict:
    common = [
        "Use Robot Lab execution_mode=external_guarded; the persisted waiting_for_operator label means waiting for the serialized guarded runner, not a human-presence gate.",
        "Standing campaign authority covers this bounded run. Keep a fresh live camera, three healthy samples, and the remote abort path ready.",
    ...
    timing = {
        ...
        "description": "Guarded repeat of the already-tested no-yaw v2 forward canary after control timing verification. One 3-second command window; arming is included by the existing runner, so measure actual engagement separately. This is not 60-second walking acceptance.",
        ...
        "parameters": {"schema": SCHEMA, "plan_id": "walking-recovery-v1-timing-forward", "robot_id": "hexapod-1",
            ...
            "prerequisites": common + [
                "Finish local runner timing checks and any required deployment separately; verify the deployed version and sample freshness behavior before running.",
            ...
            "advance_gate": "Review camera + actual engagement/tick/sample age evidence, no fall or hard trip, and stable stop. Only then queue separately bounded repeated direction trials; do not automatically extend duration.",
        ...
        loaded.append({
            ...
            "parameters": {"schema": SCHEMA, "plan_id": f"walking-recovery-v1-planted-L{leg}", "robot_id": "hexapod-1",
                ...
                "prerequisites": common + [
                    ...
                    "The serialized guarded supervisor must enforce supports, target-only motion, live cameras and fresh telemetry throughout; camera plus three advancing healthy 18/18 samples is sufficient when it conclusively establishes normal state. sysid.run_hw alone does not enforce all these gates.",
                ...
                "standing_campaign_authority": True,
                "camera_telemetry_supervision_required": True,
                "hands_on_operator_required": False,
    ...
    return {"schema": SCHEMA, "queue_payloads": [timing, *loaded],
            ...
            "future_acceptance_not_queued": {"continuous_engaged_seconds_per_run": 60, "independent_repeats": 3,
                "blocked_on": ["existing walk trial CLI accepts only3–20second command windows", "bounded pilots, trustworthy timing and calibrated motion", "a separately reviewed long-run protocol, site clearance, and guarded camera/telemetry readiness"],
