...
def hold_churn_reward(env, goal, parts, reward):
    """HOLD-mode leg-swing/duty-cycle churn price (2026-09-25, standwalk
    STATUS ~02:1x follow-up to the `hold_start_bank` FAIL-MECHANISM
    canary). That canary showed the composed-session `hold` fall gap
    (det 9/30, sto 21/30, all `hold_min_load`) is NOT an entry-state
    problem -- the same leg-fidget signature (a foot repeatedly
    unloading/re-swinging, `eval_modeseq.py`'s `duty_cycle`/
    `swing_count` fields) persists at the identical magnitude whether
    the episode starts from a real harvested post-rise pose or the
    idealized static plant. The gate's own named next lever: a
    reward-side term that prices the churn DIRECTLY, since the two
    existing hold prices both miss it -- `hold_min_load` termination
    is a delayed EMA cliff with no gradient until it fires, and
    `hold_still_gate`'s `still_h` factor is a Gaussian on MEAN qd^2
    across all 18 joints, which a single fidgeting leg among six barely
    moves. This prices the exact event `swing_count` counts (a
    previously-loaded foot going airborne) the tick it happens,
    densely, every hold tick.

    cfg `reward.k_hold_churn` (cost per liftoff event, default 0.0 =
    off, bit-exact -- the whole block, including the prev-contact
    bookkeeping, is skipped when 0 so a fresh episode's
    `_hold_churn_prev_on` state is never touched by an inert lever).
    `reward.hold_churn_load_n` (default 0.5 N, the same per-foot
    loaded-force threshold `transition_foot_drag_metric` already uses)
    sets the loaded/unloaded boundary. Scoped to `mode == "hold"`
    only (not `track`, whose reference is deliberately in motion) and
    reset at the first tick of every hold SEGMENT
    (`_step_i == _seg_entry_step`, the same marker
    `hold_min_load_terminate_grace_s` reads) so a composed session's
    prior segment (e.g. `lower`) can never masquerade as a hold-phase
    fidget the instant control re-enters hold.
    """
    k_churn = float(cfg_get(env.cfg, "reward", "k_hold_churn",
                            default=0.0))
    if k_churn <= 0.0:
        return reward
    if (goal is None or env._goal_traj is None
        ...
        return reward
    thr_n = float(cfg_get(env.cfg, "reward", "hold_churn_load_n",
                          default=0.5))
    loaded_now = []
    for i in range(6):
        ...
        if adr >= 0:
            ...
            loaded_now.append(f_n > thr_n)
        else:   # no sensor: fall back to the clearance test
            clear_i = (float(env.data.xpos[env._pad_bids[i], 2])
                       - env._pad_z_ref[i])
            loaded_now.append(
                clear_i <= PLANT_SPEC["foot_down_mm"] * 0.001)
    since_seg = env._step_i - env._seg_entry_step
    prev = getattr(env, "_hold_churn_prev_on", None)
    if since_seg <= 0 or prev is None:
        prev = [None] * 6
    flips = 0
    for i in range(6):
        if prev[i] is True and loaded_now[i] is False:
            flips += 1
    env._hold_churn_prev_on = loaded_now
    if flips > 0:
        pen = k_churn * flips
        reward -= pen
        parts["hold_churn_pen"] = parts.get("hold_churn_pen", 0.0) - pen
        parts["hold_churn_flips"] = parts.get(
            "hold_churn_flips", 0) + flips
    return reward
