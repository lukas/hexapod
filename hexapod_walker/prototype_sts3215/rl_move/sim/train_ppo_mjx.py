...
from .train_ppo_sim import (
    ...
    _parse_goal_mix, _warn_if_defaults,
)
from .mjx_train_setup import (
    _assert_gpu_physics, _env_kwargs, _init_wandb, _json_safe_config,
    _resolve_impl, _unwrap_vec, _write_training_complete_marker,
    apply_walkcurr_cert_cfg_overrides, apply_walkcurr_post_promo_schedule,
)
from .mjx_train_args import (
    _activation_fn, _clip_range_anneal_value, _fixup_log_std_final_argv,
...
from .recover_curriculum import (
    _recover_cert_bucket_plan, _recover_score_payload,
    _recover_update_admission_all, _recover_update_regression_timers,
    _restore_recover_curriculum_from_sidecar, _run_recover_cert_kind,
    _recover_episode_outcome,  # noqa: F401  (re-exported for tests)
    _recover_episode_training_error,  # noqa: F401
)
from .recover_population import (
    _RecoverPopulation,
    _recover_population_all_acked,  # noqa: F401  (re-exported for tests)
    _recover_population_all_ready,  # noqa: F401
    _recover_population_choose_candidate,  # noqa: F401
    _recover_population_record,  # noqa: F401
    _recover_population_release,  # noqa: F401
    _recover_population_start,  # noqa: F401
)
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--walk-curriculum-version", type=int, default=1,
                    ...
                    help="1 = WALKCURR_BUCKETS (walkcurr1), 2 = "
                         ...
                         "the same wz!=0 ticks the freeze does), 10 = "
                         "WALKCURR_BUCKETS_V10 recalibrated-threshold "
                         "repair of 9 (walkcurr9-canary-r1 post-mortem, "
                         "2026-09-12: frontier never certified bucket 0 "
                         "in 4 cert rounds over a full 2M budget because "
                         "the V6-authored slip/roll/height thresholds "
                         "were tuned for a different champion; 10 is "
                         "byte-identical to 9 except slip_per_m_max "
                         "1.8/2.0->6.5, peak_roll_deg_max 8.0->17.0, "
                         "height_factor_min 0.80->0.60 on every bucket's "
                         "gate, matched to THIS champion's own measured "
                         "bucket-0 operating point with margin)")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.walk_curriculum:
        ...
        if (args.init_from is not None and not args.init_from_actor_only
                ...
                and args.walk_curriculum_version not in (5, 6, 7, 8, 9, 10)):
            raise SystemExit("--walk-curriculum is a fresh-actor "
                             ...
                             "fix, 08-24) and V9/V10 (cert-metric fix + "
                             "its own threshold recalibration) ride the "
        ...
        if args.walk_curriculum_version in (4, 5, 6, 7, 8, 9, 10):
            if args.walk_curriculum_version == 10:
                from .walk_task import WALKCURR_BUCKETS_V10 as _wc_tbl
            elif args.walk_curriculum_version == 9:
    ...
    if args.walk_curriculum:
        ...
        from .walk_task import WALKCURR_BUCKETS_V10 as _WC10
        _tbl = {1: _WC1, 2: _WC2, 3: _WC3, 4: _WC4,
                5: _WC5, 6: _WC6, 7: _WC7,
                8: _WC8, 9: _WC9, 10: _WC10}[args.walk_curriculum_version]
    ...
    if args.algo == "sac":
        ...
        # branch below -- see _build_sac_model and _sac_incompatible_flags).
        _bad = _sac_incompatible_flags(
            init_from_actor_only=bool(args.init_from_actor_only),
            init_from_policy_backbone=bool(args.init_from_policy_backbone),
            gru=args.gru, gru_dual=args.gru_dual,
            gru_experts=args.gru_experts, transformer=args.transformer,
            asym_critic=args.asym_critic,
            critic_encoder=args.critic_encoder is not None,
            decleg=args.decleg, use_sde=args.use_sde,
            predictive_live=args.predictive_live,
            walk_curriculum=bool(args.walk_curriculum),
            recover_population_id=bool(args.recover_population_id),
            obs_pad_transplant=bool(args.obs_pad_transplant),
            hist_stride_transplant=bool(args.hist_stride_transplant),
            ent_coef_final=args.ent_coef_final is not None,
            log_std_final=args.log_std_final is not None,
            actor_lr_on=args.actor_lr > 0.0,
            amp_style_weight_on=args.amp_style_weight > 0.0,
            rnd_coef_on=args.rnd_coef > 0.0,
            mirror_coef_on=mirror_coef > 0.0, bc_coef_on=bc_coef > 0.0)
        if _bad:
    ...
    from .mjx_callbacks import _Track
    ...
    callbacks: list = [_Track(args, venv, run, out_path)]
    ...
    if args.walk_curriculum:
        ...
        from .walk_task import (WALKCURR_BUCKETS, WALKCURR_BUCKETS_V2,
                                ...
                                WALKCURR_BUCKETS_V9, WALKCURR_BUCKETS_V10)
        ...
        wc_table = {1: WALKCURR_BUCKETS, 2: WALKCURR_BUCKETS_V2,
                    ...
                    9: WALKCURR_BUCKETS_V9,
                    10: WALKCURR_BUCKETS_V10}[args.walk_curriculum_version]
