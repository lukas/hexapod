...
def course_charge(env, goal, info, reward, s_ref, v):
    ...
    k_course = float(cfg_get(env.cfg, "reward",
                             "k_walk_course", default=0.0))
    if k_course > 0.0 and s_ref > 1e-3:
        tau_c = max(float(cfg_get(env.cfg, "reward",
                                  "walk_course_tau_s",
                                  default=0.75)), env.dt)
        a_c = env.dt / tau_c
        env._walk_course_ema[0] += a_c * (
            float(v[0]) - env._walk_course_ema[0])
        env._walk_course_ema[1] += a_c * (
            float(v[1]) - env._walk_course_ema[1])
        ex, ey = env._walk_course_ema
        spd_c = math.hypot(ex, ey)
        v_min_c = float(cfg_get(env.cfg, "reward",
        ...
        k_cover = float(cfg_get(env.cfg, "reward",
