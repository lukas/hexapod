"""Mechanics-only tests for the promoted `cw-walk50hz-slew-smooth-s0`
walk-role cfg-set recipe (walkcurr packaging, 2026-09-21). No
rollouts, no rankings -- just checks the arg list is well-formed and
applies cleanly, per RESEARCH_RULES "Tests". Mirrors
test_cfg_recipe_walk50hz_rlonly_v2.py 1:1 for the sibling module."""
from rl_move.sim.cfg_recipe_walk50hz_slew_smooth_s0 import (
    CFG_ARGS,
    apply_to_cfg,
    build_cfg_args,
)
...
def test_differs_from_rlonly_v2_on_the_promoted_axes():
    """The whole point of this being a SEPARATE module: this
    checkpoint's own smoothness/slew-cap retune differs from its
    parent's recipe on exactly these keys."""
