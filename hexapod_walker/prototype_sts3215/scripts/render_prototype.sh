...
#   - sources its STLs from artifacts/assembly/ (built by
#     scripts/build_prototype_assembly.py)
...
#
# Usage (from anywhere):
#   ./hexapod_walker/prototype_sts3215/scripts/render_prototype.sh
#   ./hexapod_walker/prototype_sts3215/scripts/render_prototype.sh --samples 256
#   ./hexapod_walker/prototype_sts3215/scripts/render_prototype.sh --device METAL
#   ./hexapod_walker/prototype_sts3215/scripts/render_prototype.sh --out renders/prototype_side.png
#   ./hexapod_walker/prototype_sts3215/scripts/render_prototype.sh --camera-azimuth-deg 90

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PROTOTYPE_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
WALKER_DIR="$(cd "$PROTOTYPE_DIR/.." && pwd)"
...
ASSETS="artifacts/assembly"
ASSEMBLY_SRC="scripts/build_prototype_assembly.py"
...
if [[ $REBUILD -eq 1 ]]; then
    ...
    "$ROOT_DIR/run.sh" hexapod_walker/prototype_sts3215/scripts/build_prototype_assembly.py
else
